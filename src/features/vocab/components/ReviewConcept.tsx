...
import { Sparkles, Brain, Target, Clock, ArrowRight, Zap, Flame, BarChart3 } from 'lucide-react';
    ...
    <Card className="w-full h-full min-h-[180px] overflow-hidden relative group border-2 hover:border-indigo-500/50 transition-all duration-300">
      ...
      <CardHeader className="relative z-10 pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge
            variant="secondary"
            className="bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300 font-bold"
          >
            12 {t('reviewPage.stats.due', { defaultValue: 'Due' })}
          </Badge>
        </div>
        <CardTitle className="text-xl font-bold text-slate-800 dark:text-slate-100 mt-2">
          {t('reviewConcept.reviewWordsTitle', { defaultValue: 'Review Words' })}
        </CardTitle>
        <CardDescription className="font-medium text-slate-500 dark:text-slate-400">
          {t('reviewConcept.reviewWordsDesc', { defaultValue: 'Strengthen your long-term memory' })}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="relative z-10 pt-0">
        <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 mb-4">
          ...
          <span>{t('reviewConcept.durationShort', { defaultValue: '~5 mins' })}</span>
        </div>
      </CardContent>
      ...
      <CardFooter className="relative z-10 pt-0">
        <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition-all shadow-lg shadow-indigo-200 dark:shadow-indigo-900/20 group-hover:translate-y-[-2px]">
          {t('reviewConcept.startSession', { defaultValue: 'Start Session' })}{' '}
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">
              {t('reviewConcept.dashboardSubtitle', {
                defaultValue: 'Keep your streak alive! You reviewed 45 words this week.',
              })}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <span className="font-bold text-slate-700 dark:text-slate-200">
                {t('reviewPage.dashboard.streakDays', { days: 12, defaultValue: '12 Days' })}
              </span>
            </div>
            <Button variant="outline" className="rounded-full">
              {t('reviewConcept.history', { defaultValue: 'History' })}
            </Button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <StatsCard
            ...
            value="843"
            subtext={t('reviewConcept.stats.learnedSub', { defaultValue: '15 new this week' })}
          />
          <StatsCard
            icon={<Clock className="text-rose-500" />}
            label={t('reviewConcept.stats.timeSpent', { defaultValue: 'Time Spent' })}
            value="2.5h"
            subtext={t('reviewConcept.stats.timeSpentSub', { defaultValue: 'Total focus time' })}
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <ModeCard
              ...
              description={t('reviewPage.modes.quick.desc', { defaultValue: '10 words • 2 mins' })}
            ...
            <ModeCard
              title={t('reviewPage.modes.full.title', { defaultValue: 'Deep Clean' })}
              description={t('reviewPage.modes.full.desc', {
                count: 32,
                defaultValue: 'Clear all 32 due words',
              })}
            ...
            <ModeCard
              ...
              description={t('reviewPage.modes.weak.desc', {
                defaultValue: 'Focus on 5 difficult items',
              })}
          ...
          <Tabs defaultValue="due" className="w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <TabsList>
                ...
                <TabsTrigger value="learned">
                  {t('reviewPage.queue.mastered', { defaultValue: 'Recently Learned' })}
                </TabsTrigger>
              </TabsList>
            </div>
              ...
              <Card>
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {[1, 2, 3].map(i => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            {t('reviewConcept.exampleMeaning', { defaultValue: 'Hello' })}
                          </p>
                        </div>
                      </div>
                      <Badge
                      ...
                      >
                        {t('dashboard.textbook.level', { level: 1, defaultValue: 'Level 1' })}
                      </Badge>
                    </div>
                  ))}
                </div>
                <div className="p-4 text-center border-t border-slate-100 dark:border-slate-800">
                  <Button variant="ghost" className="text-slate-500">
                    {t('reviewConcept.viewAllWords', { defaultValue: 'View all 32 words' })}
                  </Button>
                </div>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="learned">
              <div className="p-8 text-center text-slate-500">
                {t('reviewConcept.emptyLearned', {
                  defaultValue: 'No recently learned words today.',
                })}
              </div>
            </TabsContent>
          </Tabs>
