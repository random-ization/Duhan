...
export const ReviewWordsCard: React.FC<ReviewWordsCardProps> = ({
  dueCount = 0,
  recommendedCount,
}) => {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between relative z-10 p-6">
        <div className="flex-1 space-y-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            >
              {t('reviewPage.card.wordsDue', {
                count: dueCount,
                defaultValue: `${dueCount} Words Due`,
              })}
            </Badge>
          </div>
          ...
          <CardDescription className="text-sm font-medium text-slate-500 dark:text-slate-400 max-w-sm">
            {t('reviewPage.card.description', {
              defaultValue:
                'Strengthen your long-term memory with spaced repetition suited for you.',
            })}
          </CardDescription>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 mt-3">
            ...
            <span>
              {t('reviewPage.card.estimatedTime', {
                min: Math.max(2, Math.ceil(dueCount * 0.5)),
                defaultValue: `Estimated time: ~${Math.max(2, Math.ceil(dueCount * 0.5))} mins`,
              })}
            </span>
          </div>
          {typeof recommendedCount === 'number' && recommendedCount > 0 && (
            <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-300 mt-1">
              {t('reviewPage.card.recommendedToday', {
                count: recommendedCount,
                defaultValue: `Recommended today: ${recommendedCount} words`,
              })}
            </div>
          )}
        </div>
        ...
        <div className="mt-5 sm:mt-0 sm:ml-6">
          ...
          >
            {t('reviewPage.card.start', { defaultValue: 'Start Review' })}{' '}
