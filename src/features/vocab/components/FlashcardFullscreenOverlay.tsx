  ...
  const content = (
    <div className="fixed inset-0 z-[95] bg-white flex flex-col">
      ...
      <div className="text-center py-2 bg-indigo-50 text-indigo-600 text-sm font-medium">
        ...
        <kbd className="px-2 py-0.5 bg-white rounded border border-indigo-200 mx-1">
          {labels.flashcardsOverlay?.spaceKey || 'Space'}
        </kbd>{' '}
        {labels.flashcardsOverlay?.keyboardInstruction || 'Press space or click the card to flip'}
      </div>

      {/* Bottom Toolbar */}
      <div className="px-6 py-4 border-t border-slate-100">{toolbar}</div>
    </div>
  );
