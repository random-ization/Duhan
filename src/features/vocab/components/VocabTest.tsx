...
export default function VocabTest({
  ...
  const submitTest = useCallback(() => {
    ...
    if (!isAllAnswered) {
      const firstMissingIdx = cards.findIndex(card => {
        const a = answers[card.id];
        return !isCardComplete(card, a);
      });
  ...
  const stats = useMemo(() => {
    ...
    for (const card of cards) {
      const a = answers[card.id];
      if (card.type === 'FILL_10') {
        totalQuestions += card.items.length;
        if (a?.type === 'FILL_10') {
          const expected =
            a.directionUsed === 'KR_TO_NATIVE'
              ? card.items.map(i => i.pair.native)
          ...
          a.filled.forEach((v, idx) => {
            if (normalizeText(v) === normalizeText(expected[idx] || '')) correctQuestions += 1;
          });
        }
      } else {
        totalQuestions += 1;
        if (card.type === 'TRUE_FALSE' && a?.type === 'TRUE_FALSE') {
          if (a.isTrue === card.correctIsTrue) correctQuestions += 1;
        } else if (card.type === 'MULTIPLE_CHOICE' && a?.type === 'MULTIPLE_CHOICE') {
          if (a.selectedIndex === card.correctIndex) correctQuestions += 1;
        } else if (card.type === 'WRITTEN' && a?.type === 'WRITTEN') {
          if (normalizeText(a.input) === normalizeText(card.answer)) correctQuestions += 1;
        }
      }
    }
              ...
              return (
                <div key={row.key} className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={disabled}
                    onClick={() =>
                      setEnabledTypes(prev => ({ ...prev, [row.key]: !prev[row.key] }))
                    }
                    className={`w-12 h-7 rounded-full relative transition-all ${
                      enabledTypes[row.key]
                        ? 'bg-blue-600'
                        : disabled
          ...
          <div className="max-w-4xl mx-auto w-full px-4 sm:px-6">
            <div className="space-y-6">
              {cards.map((card, idx) => {
                ...
                const isComplete = isCardComplete(card, answered);
                const isActive = active?.id === card.id;
                const isMissing = submitAttempted && !isComplete;
                return (
                  <div
                    key={`${card.id}:${idx}`}
                    ref={el => {
                    ...
                    className={`rounded-3xl border-2 p-6 sm:p-8 transition-all ${
                      isActive
                        ? isMissing
                          ? 'border-red-600 bg-red-50 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]'
                  ...
                  >
                    ...
                    {card.type === 'TRUE_FALSE' ? (
                      <TestCardTrueFalse
                        ...
                        answered={
                          answered?.type === 'TRUE_FALSE' ? { isTrue: answered.isTrue } : undefined
                        }
                        onSubmit={isTrue => {
                          upsertAnswer(card.id, { type: 'TRUE_FALSE', isTrue });
                          if (idx < cards.length - 1) goToCard(idx + 1);
                        }}
                      />
                    ) : card.type === 'MULTIPLE_CHOICE' ? (
                      ...
                      <TestCardWritten
                        ...
                        prompt={card.prompt}
                        answered={
                          answered?.type === 'WRITTEN' ? { input: answered.input } : undefined
                        }
                        onSubmit={input => {
        ...
        <div className="mt-8">
          ...
          <div className="space-y-6">
            {cards.map((card, idx) => {
              ...
              const correctness =
                card.type === 'FILL_10' && a?.type === 'FILL_10'
                  ? (() => {
                      const expected =
                        a.directionUsed === 'KR_TO_NATIVE'
                          ? card.items.map(i => i.pair.native)
                          : card.items.map(i => i.pair.korean);
                      const correctCount = expected.reduce((acc, exp, i) => {
                        return normalizeText(a.filled[i] || '') === normalizeText(exp || '')
                          ? acc + 1
                          : acc;
                      }, 0);
                      return {
                        correctCount,
                        totalCount: card.items.length,
                        allCorrect: correctCount === card.items.length,
                      };
                    })()
                  : card.type === 'TRUE_FALSE' && a?.type === 'TRUE_FALSE'
                    ? {
                        correctCount: a.isTrue === card.correctIsTrue ? 1 : 0,
                        totalCount: 1,
                        allCorrect: a.isTrue === card.correctIsTrue,
                      }
                    : card.type === 'MULTIPLE_CHOICE' && a?.type === 'MULTIPLE_CHOICE'
                      ? {
                          correctCount: a.selectedIndex === card.correctIndex ? 1 : 0,
                          totalCount: 1,
                          allCorrect: a.selectedIndex === card.correctIndex,
                        }
                      : card.type === 'WRITTEN' && a?.type === 'WRITTEN'
                        ? {
                            correctCount:
                              normalizeText(a.input) === normalizeText(card.answer) ? 1 : 0,
                            totalCount: 1,
                            allCorrect: normalizeText(a.input) === normalizeText(card.answer),
                          }
                        : null;
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {correctness && (
                      ...
                      >
                        {card.type === 'FILL_10'
                          ? `${correctness.correctCount}/${correctness.totalCount}`
                          : correctness.allCorrect
                            ? language === 'zh'
                              ? '正确'
                              : 'Correct'
                  ...
                  {card.type === 'TRUE_FALSE' ? (
                    ...
                    <TestCardFill10
                      ...
                      answered={
                        a?.type === 'FILL_10'
                          ? { filled: a.filled, directionUsed: a.directionUsed }
                          : undefined
                      }
                      onSubmit={() => {}}
                    />
