...
export const useCanvasAnnotation = (
    ...
    const canvasQuery = useQuery(convexApi.canvas.getCanvas, {
        targetId,
        targetType,
        pageIndex
    });

    const saveMutation = useMutation(convexApi.canvas.saveCanvas);
    ...
    useEffect(() => {
        ...
        if (loadedRef.current !== key && canvasQuery !== undefined) {
            ...
            if (canvasQuery) {
                setCanvasData(canvasQuery.data as CanvasData);
            } else {
                setCanvasData(null);
            }
