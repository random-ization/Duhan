...
export default function ModernDashboard() {
  ...
  const userStats = useQuery(qRef<NoArgs, UserStats | null>('userStats:getStats'));
  ...
  const stats: LearningStats = userStats
    ? {
        ...
        todayActivities: userStats.todayActivities || {
          wordsLearned: 0,
          readingsCompleted: 0,
  ...
  const nextExam = useMemo(() => {
    const now = new Date();
    const upcoming = TOPIK_EXAMS.filter(exam => exam.date > now).sort(
    ...
    return {
      name: exam.name,
      date: exam.date.toLocaleDateString('zh-CN', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      }),
  ...
  const goalProgress = {
    ...
    listenings: {
      target: dailyGoals.listenings,
      completed: stats.todayActivities.listeningsCompleted,
    },
  };
  ...
  if (!user) {
    return <div className="p-8 text-center text-gray-500">{t('login')}</div>;
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {t('dashboard.greeting.goalPart1')}{' '}
            <span className="font-bold text-indigo-600">{t('dashboard.greeting.goalPart2')}</span>{' '}
            {t('dashboard.greeting.goalPart3')}
          </p>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                {t('dashboard.common.streak')}
              </div>
              <div className="font-bold text-slate-800 text-sm">
                {t('dashboard.common.days', { count: stats.streak || 1 })}
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="text-center sm:text-left">
                  ...
                  <p className="text-indigo-200 text-sm max-w-md">
                    <Trans i18nKey="dashboard.banner.upgradeDesc">
                      您当前使用的是<span className="text-white font-bold">免费版</span>
                      ，仅可访问部分基础课程。
                    </Trans>
                    <br />
                    {t('dashboard.banner.unlockDesc')}
                  </p>
                </div>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center">
                ...
                <div className="flex-1 text-center md:text-left w-full">
                  ...
                  <p className="text-slate-400 text-sm mb-6">
                    {lastCourse.displayLevel && `${lastCourse.displayLevel} · `}
                    {lastCourse.volume && `${lastCourse.volume} · `}
                    {lastCourse.unit}
                  </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              {
                icon: Target,
                label: t('dashboard.tools.topik_mock'),
                color: 'text-rose-600 bg-rose-50',
                desc: t('dashboard.tools.mock_desc'),
                path: '/topik',
              },
              {
                icon: BookOpen,
                label: t('dashboard.tools.flashcards'),
                color: 'text-amber-600 bg-amber-50',
                desc: t('dashboard.tools.flashcards_desc'),
                path: '/dashboard',
              },
              {
                icon: Clock,
                label: t('dashboard.tools.listening'),
                color: 'text-blue-600 bg-blue-50',
                desc: t('dashboard.tools.listening_desc'),
                path: '/dashboard',
              },
              {
                icon: BarChart3,
                label: t('dashboard.tools.mistakes'),
                color: 'text-emerald-600 bg-emerald-50',
                desc: t('dashboard.tools.mistakes_desc'),
                path: '/dashboard',
              },
              {
                icon: Headphones,
                label: t('notebook') || '我的笔记',
              ...
              >
                <div
                  className={`w-10 h-10 ${item.color} rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}
                >
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                {
                  id: 'words',
                  title: t('dashboard.common.goalWords', { target: goalProgress.words.target }),
                  done: goalProgress.words.completed >= goalProgress.words.target,
                },
                {
                  id: 'readings',
                  title: t('dashboard.common.goalReadings', {
                    target: goalProgress.readings.target,
                  }),
                  done: goalProgress.readings.completed >= goalProgress.readings.target,
                },
                {
                  id: 'listenings',
                  title: t('dashboard.common.goalListenings', {
                    target: goalProgress.listenings.target,
                  }),
                  done: goalProgress.listenings.completed >= goalProgress.listenings.target,
                },
              ].map(goal => (
                <div
                  key={goal.id}
                ...
                >
                  ...
                  <span
                    className={`text-sm font-medium ${goal.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}
                  >
                    {goal.title}
                  </span>
                </div>
            ...
            <button className="w-full mt-4 py-2 text-xs font-bold text-slate-400 hover:text-indigo-600 flex items-center justify-center gap-1 transition-colors group">
              {t('dashboard.common.viewAllPlans')}{' '}
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t('dashboard.common.settings.words')}
                </label>
                <input
                  type="number"
                  min="1"
                  ...
                  onChange={e =>
                    setDailyGoals({ ...dailyGoals, words: parseInt(e.target.value) || 1 })
                  }
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t('dashboard.common.settings.readings')}
                </label>
                <input
                  type="number"
                  min="1"
                  ...
                  onChange={e =>
                    setDailyGoals({ ...dailyGoals, readings: parseInt(e.target.value) || 1 })
                  }
                  className="w-full px-4 py-2 border rounded-lg"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t('dashboard.common.settings.listenings')}
                </label>
                <input
                  type="number"
                  min="1"
                  ...
                  onChange={e =>
                    setDailyGoals({ ...dailyGoals, listenings: parseInt(e.target.value) || 1 })
                  }
                  className="w-full px-4 py-2 border rounded-lg"
                />
              </div>
