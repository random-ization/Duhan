...
import { useTopikQuestions, TopikExamWithUrl } from '../hooks/useTopikQuestions';
...
const FONT_SANS = 'font-sans'; // 无斜体字体 (用于题目)
// 深色边框

const TopikModule: React.FC<TopikModuleProps> = ({
  exams,
  language,
  history: _history,
  onSaveHistory,
  ...
  const [localMenuPosition, setLocalMenuPosition] = useState<{ top: number; left: number } | null>(
    null
  );
  ...
  const handleTextSelection = (e: React.MouseEvent) => {
    ...
    const exactMatch = currentAnnotations.find(a => a.text === text);
  ...
  const submitExam = useCallback(() => {
    ...
    setView('RESULT');
  }, [currentExam, currentQuestions, userAnswers, onSaveHistory, clearExamStorage]);
  ...
  const renderHighlightedText = (text: string, itemContextKey: string) => {
    ...
    while (i < text.length) {
      ...
      if (currentAnn) {
        const isActive =
          activeAnnotationId === currentAnn.id || editingAnnotationId === currentAnn.id;
  ...
  const renderPassage = (q: TopikQuestion) => {
    ...
    if (q.layout === 'NEWS_HEADLINE' && q.passage) {
      return (
        ...
        >
          <h3
            className={`${FONT_SERIF} font-bold text-xl text-slate-900 leading-snug tracking-tight text-center select-text`}
          >
    ...
    if (
      q.passage &&
      (q.id === 9 || q.id === 10 || q.passage.includes('|') || q.passage.includes('구분'))
    ) {
      return (
    ...
    return (
      ...
      >
        ...
        <div className="flex gap-2 mb-3">
          <span
            className={`text-[19px] font-bold text-slate-900 ${FONT_SERIF} min-w-[28px] pt-0.5`}
          >
            {q.id}.
          </span>
        ...
        <div className="pl-9">
          {q.optionImages && q.optionImages.some(img => img) ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              {q.optionImages.map((img, idx) => (
                <button
                  ...
                  disabled={isReview}
                  className={`relative border-2 rounded-lg p-2 transition-all hover:bg-slate-50 ${
                    myAnswer === idx
                ...
                >
                  <span
                    className={`absolute top-2 left-2 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${myAnswer === idx ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'}`}
                  >
                    {getCircleNumber(idx)}
                  </span>
                  {img ? (
                    <img src={img} className="w-full h-32 object-contain mx-auto" />
                  ) : (
            ...
            <div
              className={`grid ${q.options.some(o => o.length > 20) ? 'grid-cols-1 gap-3' : 'grid-cols-2 gap-x-8 gap-y-2'}`}
            >
              {q.options.map((opt, idx) => {
                let itemClass =
                  'flex items-start gap-2 cursor-pointer p-1.5 rounded -ml-2 transition-colors border border-transparent';

                // 选中样式
                if (!isReview && myAnswer === idx)
                  itemClass += ' bg-indigo-50/80 border-indigo-200';
                ...
                if (isReview) {
                  if (idx === q.correctAnswer)
                    ...
                    itemClass += ' bg-red-50 border-red-200 text-red-800 line-through opacity-80';
                ...
                return (
                  <label key={idx} className={itemClass}>
                    ...
                    />
                    <span
                      className={`text-[15px] font-sans ${myAnswer === idx ? 'font-bold text-slate-900' : 'text-slate-500'}`}
                    >
                      {getCircleNumber(idx)}
                    </span>
                    <span
                    ...
                    {isReview && idx === q.correctAnswer && (
                      <Check className="w-4 h-4 text-green-600 ml-auto shrink-0" />
                    )}
                    {isReview && myAnswer === idx && myAnswer !== q.correctAnswer && (
                      <X className="w-4 h-4 text-red-600 ml-auto shrink-0" />
                    )}
                  </label>
                );
              })}
  ...
  const renderGroup = (group: TopikQuestion[]) => {
    ...
    // 指引文 (e.g. [1~2] ...)
    const rangeText =
      group.length > 1 ? `[${leader.id}~${group[group.length - 1].id}]` : `[${leader.id}]`;
    ...
    return (
      <div key={`group-${leader.id}`} className="mb-12 break-inside-avoid">
        ...
        <div className={`flex flex-col ${hasSharedPassage ? 'lg:flex-row lg:gap-8' : ''}`}>
          ...
          {hasSharedPassage && (
            <div className="lg:w-1/2 shrink-0 mb-8 lg:mb-0">{renderPassage(leader)}</div>
          )}

          {/* Right Column: Questions */}
          <div className="flex-1">{group.map(q => renderQuestion(q, !hasSharedPassage))}</div>
        </div>
      </div>
    );
  };
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur text-white p-3 shadow-lg flex items-center gap-4 z-50 border-t border-slate-700">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono font-bold tracking-widest uppercase">
            {labels.listeningTest || 'Listening Test'}
          </span>
        </div>
        <button
          onClick={toggle}
          className="w-10 h-10 bg-white text-slate-900 rounded-full flex items-center justify-center hover:bg-indigo-100 transition-colors shadow-lg active:scale-95"
        >
          {playing ? (
        ...
        <div className="flex-1 h-1.5 bg-slate-700 rounded-full overflow-hidden cursor-pointer group">
          <div
            className="bg-indigo-400 h-full transition-all duration-100 relative group-hover:bg-indigo-300"
      ...
      <div className="flex flex-col h-screen h-[100dvh] bg-slate-200 overflow-hidden">
        ...
        <div className="bg-slate-800 text-white h-14 flex items-center justify-between px-6 shadow-md z-20 shrink-0">
          <div className="flex items-center gap-4">
            {view === 'REVIEW' && (
              <button onClick={() => setView('HISTORY_LIST')}>
          ...
          {view === 'EXAM' ? (
            <div
              className={`text-xl font-mono font-bold flex items-center ${timeLeft < 300 ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}
            >
          ...
          <div className="w-32 flex justify-end">
            {view === 'EXAM' && (
              <button
                onClick={submitExam}
                className="bg-white text-slate-900 px-4 py-1.5 rounded font-bold text-sm hover:bg-indigo-50"
              >
                {labels.submit}
              </button>
            )}
        ...
        <div className="flex-1 flex min-h-0 relative">
          {/* Main Paper Area */}
          <div
            className="flex-1 overflow-y-auto overflow-x-hidden flex justify-center p-4 md:p-8 relative bg-slate-200"
              ...
              <div className={`flex-1 px-8 md:px-12 pb-12 ${view === 'EXAM' ? 'select-none' : ''}`}>
                ...
                {!questionsLoading &&
                  ...
                  questionGroups.map((group, idx) => (
                    <React.Fragment key={idx}>{renderGroup(group)}</React.Fragment>
                  ))}
              </div>
    ...
    return (
      <div className="min-h-screen bg-slate-100 py-10 flex justify-center overflow-y-auto">
        <div className="bg-white w-full max-w-[800px] shadow-2xl p-16 border border-slate-300 flex flex-col relative h-fit min-h-[1000px]">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-slate-50 text-[120px] font-black opacity-50 pointer-events-none select-none">
            TOPIK
          </div>
          <div className="text-center border-b-4 border-double border-black pb-8">
            ...
            <div className="text-3xl font-bold font-serif bg-slate-900 text-white inline-block px-6 py-2 rounded-full">
              B형
            </div>
          </div>
          <div className="flex-1 flex flex-col justify-center gap-12 px-8 mt-12">
            <div className="bg-slate-50 border-2 border-slate-200 p-8 text-center">
              <h3 className="font-bold text-lg mb-6 border-b border-slate-300 pb-4 inline-block px-8">
                유 의 사 항 (Information)
              </h3>
          ...
          <button
            onClick={beginTest}
            className="w-full py-6 bg-black text-white text-2xl font-bold font-serif hover:bg-slate-800 transition-colors shadow-xl mt-12 tracking-widest"
          >
            {labels.startExam}
          </button>
        </div>
              ...
              >
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                    {labels.round} {exam.round}
                  </span>
                </div>
                <button
                  onClick={() => startExam(exam)}
                  className="w-full mt-2 py-2 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-700"
                >
                  {labels.startExam}
                </button>
              </div>
    ...
    <div className="flex items-center justify-center h-screen h-[100dvh] bg-slate-100">
      <div className="bg-white p-12 rounded-2xl shadow-xl text-center">
        ...
        {currentReviewAttempt && (
          <div className="text-xl text-indigo-600 font-bold mb-8">
            Score: {currentReviewAttempt.score} / {currentReviewAttempt.maxScore}
          </div>
        )}
        <div className="flex gap-4 justify-center">
          ...
          <button
            ...
            className="px-6 py-3 bg-slate-200 text-slate-700 rounded-lg font-bold"
          >
            {labels.backToList}
          </button>
        </div>
