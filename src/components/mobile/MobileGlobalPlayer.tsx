    ...
    <>
      {isMinimized ? (
        <div className="fixed bottom-20 left-4 right-4 z-50">
          <div
            className="bg-slate-900/95 backdrop-blur-md rounded-2xl p-3 flex items-center shadow-2xl border border-white/10"
          ...
          >
            <img
              src={artworkSrc}
              className="w-10 h-10 rounded-lg bg-slate-800 object-cover shrink-0"
              alt="Art"
              loading="lazy"
            />
            <div className="flex-1 min-w-0 mx-3">
              <h4 className="text-white font-bold text-sm truncate">{activeEpisode.title}</h4>
              <p className="text-slate-400 text-xs truncate">{activeEpisode.channelTitle}</p>
            </div>
        ...
        <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
          ...
          <div className="flex-1 px-8 flex flex-col justify-center pb-12 overflow-y-auto">
            <div className="w-full aspect-square rounded-[2rem] bg-slate-100 shadow-2xl mb-12 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-12">
              <button type="button" className="text-slate-400 p-2 hover:text-slate-600">
              ...
              <div className="flex items-center gap-6">
                <button
                  type="button"
                  className="text-slate-900 p-2 active:scale-90 transition-transform"
                  onClick={() => skip(-15)}
                ...
                <button
                  type="button"
                  className="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center text-white shadow-xl shadow-indigo-200 hover:scale-105 active:scale-95 transition-all"
                  onClick={togglePlay}
                  aria-label={isPlaying ? 'Pause' : 'Play'}
                >
                  {isPlaying ? (
