      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <textarea
            ...
            placeholder={t('community.composerPlaceholder', { defaultValue: '分享你的学习心得...' })}
          ...
          {selectedImages.length > 0 && (
            <div className="flex gap-2 mt-2 mb-3">
              {selectedImages.map((img, i) => (
                <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-k-line/10">
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-k-line/5">
            ...
            >
              <ImageIcon size={18} />
              <span className="text-[12px] font-bold">{t('common.image', { defaultValue: '图片' })}</span>
            </button>
            <input 
              type="file" 
              ref={fileInputRef} 
...
const CommunityItem = ({
  ...
  const handleLike = async () => {
    try {
      if (item.likedByMe) {
        await unlikeActivity({ activityId: item.activityId, kind: 'event' });
      } else {
        await likeActivity({ activityId: item.activityId, kind: 'event' });
      }
    } catch (err) {
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-xl bg-k-bg2 overflow-hidden border border-k-line/10 shrink-0">
          {item.actorAvatar ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-lg">{tone.emoji}</div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] font-bold text-k-sub/50 uppercase tracking-widest mt-0.5">{item.time}</div>
        </div>
        ...
        {item.attachment && (
          <div className="mt-4 p-4 rounded-2xl bg-k-bg2/40 border border-k-line/10 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-k-card flex items-center justify-center text-xl shadow-sm">
              {item.attachment.emoji || '📝'}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[13px] font-black text-k-ink truncate">{item.attachment.title}</div>
              <div className="text-[11px] font-bold text-k-sub/60 truncate">{item.attachment.description}</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="px-5 pb-10">
        {activeTab === 'feed' ? (
            ...
            {showGuidelines && (
              <Card pad={20} className="bg-k-ink text-k-bg mb-6 relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[11px] font-black uppercase tracking-widest text-k-butter">社区准则</span>
                </div>
            ...
            <AnimatePresence>
              {communityFeed === undefined ? (
                ...
                <div className="py-20 text-center">
                  ...
                  <div className="text-[13px] font-black text-k-sub/40 uppercase tracking-widest">暂无好友动态</div>
                </div>
              ) : (
                communityFeed.map((item: any) => (
                  <motion.div
                    key={item.activityId}
          ...
          <div className="space-y-4">
              ...
              <div className="relative mt-2">
                ...
                {searchQuery.length >= 2 && (
                  <div className="mt-4 space-y-3">
                    {searchUsers === undefined && <div className="py-2 text-center text-[12px] text-k-sub animate-pulse">Searching...</div>}
                    {searchUsers?.length === 0 && <div className="py-2 text-center text-[12px] text-k-sub">No users found</div>}
                      ...
                      return (
                        <div key={key} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[10px] font-bold text-k-sub/50">{item.friendCode}</div>
                          </div>
                          <button
                            disabled={disabled}
                          ...
                          >
                            {relation === 'already_friends' ? t('friends.isFriend', { defaultValue: '已是好友' }) : 
                             relation === 'already_requested' ? t('friends.sent', { defaultValue: '已申请' }) : 
            ...
            <Card pad={20} className="bg-gradient-to-br from-k-ink to-k-ink2 text-k-bg shadow-k-sh-lg mb-6 overflow-hidden relative">
               ...
               <div className="text-[12px] font-bold text-k-bg/60">前 10 名用户将进入下一轮精英榜单</div>
            </Card>

            <SectionHead title={t('leaderboard.topLearners', { defaultValue: '顶尖学习者' })} kanji="榜" />
                ...
                {weeklyTop?.map((p: any, i: number) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      i === 0 ? "bg-k-gold text-white" : 
                      i === 1 ? "bg-k-ink/20 text-k-ink" :
                      i === 2 ? "bg-k-butter text-k-ink" : "bg-k-bg2 text-k-sub/50"
                    )}>
                      {i + 1}
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-k-bg2 overflow-hidden border border-k-line/10 shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={cn("text-[14px] truncate tracking-tight", p.isMe ? "font-black text-k-ink" : "font-bold text-k-ink2")}>
                        {p.name}
                        {p.isMe && <span className="ml-1 opacity-50">(我)</span>}
                      </div>
                      <div className="text-[10px] font-bold text-k-sub/40 uppercase tracking-widest">Learner</div>
                    </div>
