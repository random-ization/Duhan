    ...
    <div className="min-h-screen bg-muted/30 pb-[130px] flex flex-col font-sans">
        ...
        <div className="px-5 pt-[calc(env(safe-area-inset-top)+16px)] pb-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" className="-ml-3 rounded-full" onClick={() => navigate('/practice')}>
                <ArrowLeft className="w-6 h-6 text-foreground" />
              </Button>
              <h1 className="text-2xl font-black text-foreground">{t('notes.v2.page.titleAllNotes', { defaultValue: 'Smart Notebook' })}</h1>
            </div>
            <Button variant="ghost" size="icon" onClick={handleCreateNote} className="bg-primary/10 text-primary rounded-full hover:bg-primary/20">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder={t('notes.v2.page.searchPlaceholder', { defaultValue: 'Search quote or note...' })}
      ...
      <div className="px-5 py-6 space-y-8 flex-1">
          ...
          <div className="relative z-10 flex flex-col gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-bold text-indigo-100">{t('notes.v2.page.todayReviewTitle', { defaultValue: 'Today’s Review Queue' })}</span>
              </div>
              <p className="text-3xl font-black">{pendingReviewCount} <span className="text-lg font-medium text-indigo-200 opacity-80">{t('notes.v2.context.pendingReview', { defaultValue: 'Pending' })}</span></p>
            </div>
            <Button variant="secondary" onClick={() => navigate('/review')} className="w-full rounded-2xl h-12 font-black text-indigo-600 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transition-all">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-black text-foreground">{t('notes.picker.myNotebooks', { defaultValue: 'My Notebooks' })}</h2>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-5 px-5 snap-x">
            <Button
                variant="outline"
                onClick={() => { setActiveNotebookId(null); setSelectedPageId(null); }}
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <Folder className={`w-6 h-6 ${activeNotebookId === null ? 'text-primary fill-primary/20' : 'text-muted-foreground'}`} />
                  <span className="text-xs font-bold bg-muted px-2 py-0.5 rounded-md">{searchResult.items.length}</span>
                </div>
                <span className="font-bold truncate w-full text-left text-sm">{t('notes.tabs.all', { defaultValue: 'All' })}</span>
            </Button>
            {notebooksResult.notebooks.map(nb => (
              <Button
                key={nb.id}
                variant="outline"
                onClick={() => { setActiveNotebookId(nb.id); setSelectedPageId(null); }}
                className={`snap-start shrink-0 w-[140px] h-24 flex flex-col items-start justify-between p-4 rounded-2xl border-2 transition-all ${
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <Folder className={`w-6 h-6 ${activeNotebookId === nb.id ? 'text-primary fill-primary/20' : 'text-muted-foreground'}`} />
                  <span className="text-xs font-bold bg-muted px-2 py-0.5 rounded-md">{nb.noteCount}</span>
                </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-black text-foreground">
            {activeNotebookId ? notebooksResult.notebooks.find(n => n.id === activeNotebookId)?.title : t('notes.v2.context.allNotes', { defaultValue: 'All Notes' })}
          </h2>
          ...
          {searchResult.items.length === 0 ? (
            <div className="py-12 flex flex-col items-center justify-center text-center px-6 border-2 border-dashed border-border rounded-3xl bg-card">
              ...
              <p className="text-muted-foreground font-medium">{t('notes.v2.context.noNotebooksHint', { defaultValue: 'No notes here.' })}</p>
            </div>
          ) : (
              ...
              {searchResult.items.map(item => {
                ...
                return (
                  <Button
                    key={item.id}
                    variant="ghost"
                    onClick={() => { setSelectedPageId(item.id); setEditorExpanded(false); handleEditorOpenChange(true); }}
                  ...
                  >
                    <div className="flex justify-between items-start w-full mb-3 gap-2">
                       <div className="flex items-center gap-2">
                         ...
                         <span className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${cardType.badgeClass}`}>
                           {cardType.label}
                         </span>
                       </div>
                       <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${status.className}`}>
                         {status.label}
                       </span>
                    </div>
                    ...
                    {item.noteKind === 'quote_card' && quoteHtml ? (
                      <blockquote className={`w-full mb-3 rounded-xl px-3 py-2.5 text-sm font-medium text-foreground line-clamp-3 ${QUOTE_CARD_RICH_CLASS} ${RICH_TEXT_CLASS}`} dangerouslySetInnerHTML={{ __html: quoteHtml }} />
                    ) : null}

                    {noteHtml ? (
                      <div className={`w-full text-muted-foreground text-sm flex-grow line-clamp-2 text-left leading-relaxed ${RICH_TEXT_CLASS}`} dangerouslySetInnerHTML={{ __html: item.noteKind === 'quote_card' ? noteHtml : snippetHtml || noteHtml }} />
                    ) : (
                      <p className="w-full text-muted-foreground text-sm flex-grow text-left">
                        {t('notes.v2.page.clickToViewAndEdit', { defaultValue: 'Click to view and edit content' })}
                      </p>
                    )}

                    <div className="flex gap-2 mt-4 pt-3 border-t border-border w-full flex-wrap items-center">
                      <span className="text-[11px] font-bold text-muted-foreground uppercase">{formatTime(item.updatedAt, dateLocale)} · {toSourceLabel(item.sourceModule, t)}</span>
                    </div>
                  </Button>
                );
              })}
               ...
               <div className="px-5 pt-8 pb-4 flex items-center justify-between border-b border-border shrink-0">
                 <div className="flex flex-col">
                   ...
                   <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1 mt-1">
                     ...
                     {saveState === 'saving' ? t('notes.v2.page.saveState.saving', { defaultValue: 'Saving...' }) : 
                      saveState === 'error' ? t('notes.v2.page.saveState.error', { defaultValue: 'Save failed' }) : 
                      t('notes.v2.page.saveState.saved', { defaultValue: 'Saved' })}
                   </span>
                 </div>
                 
                 <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={() => setEditorExpanded(!editorExpanded)} className="text-muted-foreground w-9 h-9">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent unstyled className="z-[60] absolute right-4 top-12 min-w-[160px] rounded-2xl border border-border bg-card p-1.5 shadow-xl">
                        {selectedSearchItem?.sourceRef && (
                          <div onClick={handleOpenSource} className="w-full flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold hover:bg-muted active:bg-muted cursor-pointer">
                            <ArrowLeft className="w-4 h-4" /> {t('notes.v2.page.viewSource', { defaultValue: 'View Source' })}
                          </div>
                        )}
                        <div onClick={() => { handleDeletePage(selectedPageId!); handleEditorOpenChange(false); }} className="w-full flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-destructive hover:bg-destructive/10 active:bg-destructive/10 cursor-pointer">
                          <Trash2 className="w-4 h-4" /> {t('notes.detail.deleteTitle', { defaultValue: 'Delete' })}
                        </div>
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <Button variant="ghost" size="icon" onClick={() => handleEditorOpenChange(false)} className="bg-muted text-muted-foreground rounded-full ml-1 w-9 h-9">
                      <X className="w-4 h-4" />
                    </Button>
                 </div>
               </div>
               ...
               <div className="flex-1 overflow-y-auto px-5 py-4">
                 {!selectedPagePayload ? (
                   <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                     ...
                     <p className="text-sm font-medium">{t('notes.v2.editor.loading', { defaultValue: 'Loading editor...' })}</p>
                   </div>
                 ) : (
                   <div className="flex flex-col gap-4 pb-10">
                     ...
                     {selectedIsQuoteCard ? (
                       <div className="space-y-4">
                         <div className={`rounded-2xl border border-border p-4 ${QUOTE_CARD_RICH_CLASS}`}>
                           ...
                           {toRichHtml(quoteText) ? (
                             ...
                             <blockquote className="whitespace-pre-wrap text-[15px] font-medium leading-relaxed text-foreground">
                               {t('notes.v2.page.quoteMissing', { defaultValue: 'Original quote not found' })}
                             </blockquote>
                           )}
                         </div>
                         <div>
                           ...
                           <OfficialTiptapEditor
                             ...
                             placeholder={t('notes.v2.page.quoteEditorPlaceholder', { defaultValue: 'Write your understanding...' })}
                             preset="study"
                           />
                         </div>
                       </div>
                       ...
                       <OfficialTiptapEditor
                         value={editorDoc}
                         onChange={setEditorDoc}
                         placeholder={t('notes.v2.page.editorPlaceholder', { defaultValue: 'Start writing your thoughts here...' })}
                         preset="full"
                       />
                     )}
                   </div>
