...
export default function MobileCommunityProfilePage() {
  ...
  return (
    <PageShell>
      <div
        style={{ padding: '14px 16px 20px', paddingTop: 'calc(env(safe-area-inset-top) + 14px)' }}
      >
        <Card pad={20}>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            {[
              {
                label: t('qa.questionsAsked', { defaultValue: 'Questions' }),
                value: profile.questionCount,
              },
              {
                label: t('qa.answersGiven', { defaultValue: 'Answers' }),
                value: profile.answerCount,
              },
              {
                label: t('qa.acceptanceRate', { defaultValue: 'Acceptance' }),
                value: formatPercent(profile.acceptanceRate),
              },
              {
                label: t('qa.totalVotes', { defaultValue: 'Votes' }),
                value: profile.totalVoteScore,
              },
            ].map(stat => (
              <div key={stat.label} style={{ background: KT.bg2, borderRadius: 18, padding: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: KT.sub }}>{stat.label}</div>
                <div style={{ marginTop: 4, fontSize: 20, fontWeight: 800, color: KT.ink }}>
                  {stat.value}
                </div>
              </div>
            ))}
              ...
              profile.recentQuestions.map(question => (
                ...
                >
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span style={{ fontSize: 11, color: KT.sub }}>
                      {formatRelativeTime(question.createdAt)}
                    </span>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: KT.ink }}>
                    {question.title}
                  </div>
                </button>
              ))
