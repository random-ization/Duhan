...
export const MobileExamSession: React.FC<MobileExamSessionProps> = ({
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-muted overflow-hidden">
      ...
      <div className="bg-card border-b border-border px-4 py-3 flex items-center justify-between shadow-sm z-20 shrink-0">
        <div className="flex items-center gap-3">
          <div
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-full font-mono font-bold text-sm tabular-nums transition-colors',
              isLowTime ? 'bg-red-50 text-red-600 animate-pulse' : 'bg-muted text-muted-foreground'
            )}
          >
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-5 pb-24">
          <div className="bg-card rounded-2xl shadow-sm border border-border p-5 md:p-6 min-h-[50vh]">
            <QuestionRenderer
      ...
      <div className="bg-card border-t border-border px-4 py-3 pb-safe flex items-center justify-between shrink-0 z-20">
        <Button
          variant="ghost"
          size="icon"
          onClick={handlePrev}
        ...
        <Button
          ...
          className="flex flex-col items-center gap-1 h-auto py-2 px-4 hover:bg-muted rounded-xl"
        >
          <div className="flex items-center gap-2">
            <span className="text-2xl font-black text-foreground">{currentQuestionIndex + 1}</span>
