...
import { USER_STATS } from '../../utils/convexRefs';
...
function useRouteTitle() {
  ...
  // 核 · 主页 (HOME)
  if (p.includes('/dashboard/course') || p.endsWith('/course')) return { title: '今日', kanji: '今', breadcrumb: 'TODAY' };
  ...
  // 會 · 社区 (SOCIAL)
  if (p.includes('/leaderboard')) return { title: '排行榜', kanji: '榜', breadcrumb: 'LEADERBOARD' };
  ...
  if (p.includes('/history')) return { title: '学习记录', kanji: '錄', breadcrumb: 'LIBRARY' };
  ...
  return (
    <div className="flex h-[60px] shrink-0 items-center gap-[18px] border-b border-k-line bg-k-bg px-7">
      <div className="flex items-baseline gap-2.5">
        {kanji && (
          <span className="font-k-serif text-[22px] font-medium text-k-crimson">
            {kanji}
          </span>
        )}
        <span className="text-[18px] font-black tracking-tight text-k-ink">
          {title}
        </span>
        {breadcrumb && (
