    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-2xl font-black text-foreground">
          {t('topikWriting.report.evaluatingTitle', { defaultValue: 'AI is evaluating...' })}
        </h2>
        <p className="text-muted-foreground font-medium text-sm max-w-xs">
          {t('topikWriting.report.evaluatingDesc', {
            defaultValue:
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span className="text-[10px] text-muted-foreground">
        {t('topikWriting.report.maxScore', { score: maxScore, defaultValue: `Max ${maxScore}` })}
      </span>
    </div>
  );
};
...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const pct = questionMaxScore > 0 ? Math.round((evaluation.score / questionMaxScore) * 100) : 0;
        ...
        <div className="border-t border-border divide-y divide-border">
          ...
          <div className="p-5 space-y-3">
            ...
            {DIMENSION_CONFIG.flatMap(dim => {
              ...
              return [
                <DimensionBar key={dim.key} label={dim.label} value={val} color={dim.color} />,
              ];
            })}
          </div>
...
export const WritingEvaluationReport: React.FC<WritingEvaluationReportProps> = ({
  ...
  const elapsedMs =
    session.completedAt && session.startTime ? session.completedAt - session.startTime : null;
  ...
  const passLabel =
    pct >= 80
      ? t('topikWriting.report.passExcellent', { defaultValue: 'Excellent 🎉' })
      : pct >= 60
        ? t('topikWriting.report.passGood', { defaultValue: 'Pass ✅' })
