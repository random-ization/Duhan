...
const MarkdownRenderer: React.FC<{ content: string }> = ({ content }) => {
    return (
        <ReactMarkdown
            ...
            components={{
                h1: ({ ...props }) => <h1 className="text-2xl font-black mt-6 mb-4 text-foreground" {...props} />,
                ...
                p: ({ ...props }) => <p className="mb-4 leading-relaxed text-muted-foreground font-medium" {...props} />,
                strong: ({ ...props }) => <strong className="font-black text-foreground bg-yellow-200/40 px-1 rounded dark:bg-yellow-900/40 dark:text-yellow-100" {...props} />,
                ul: ({ ...props }) => <ul className="list-disc pl-5 mb-4 space-y-2 text-muted-foreground font-medium" {...props} />,
                ol: ({ ...props }) => <ol className="list-decimal pl-5 mb-4 space-y-2 text-muted-foreground font-medium" {...props} />,
                li: ({ ...props }) => <li className="" {...props} />,
                blockquote: ({ ...props }) => <blockquote className="border-l-4 border-slate-900 dark:border-border pl-4 italic text-muted-foreground bg-muted p-2 rounded-r-lg mb-4" {...props} />,
                code: ({ ...props }) => <code className="bg-slate-100 dark:bg-slate-800 text-pink-600 dark:text-pink-400 font-bold px-1.5 py-0.5 rounded text-sm border-b-2 border-slate-300 dark:border-slate-700" {...props} />
            }}
        >
            {content}
        </ReactMarkdown>
...
const GrammarDetailPane: React.FC<GrammarDetailPaneProps> = ({
    ...
    const getTypeStyles = () => {
        switch (grammar.type) {
            ...
            case 'CONNECTIVE':
                return { bg: 'bg-amber-50 dark:bg-amber-950', text: 'text-amber-600 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-800' };
            default:
    ...
    const rulesObject = (grammar.conjugationRules || grammar?.construction || {}) as Record<string, unknown>;
            ...
            <div className="max-w-4xl w-full mx-auto bg-card rounded-2xl border-2 border-slate-900 dark:border-border shadow-[8px_8px_0px_0px_#0f172a] dark:shadow-[8px_8px_0px_0px_rgba(148,163,184,0.26)] p-10 mt-2 mb-10">
                ...
                <header className="border-b-2 border-slate-200 dark:border-slate-800 pb-6 mb-8">
                    <div className="flex items-center gap-2 mb-3">
                        <span className={`text-xs font-black px-2 py-1 border-2 rounded uppercase ${typeStyles.bg} ${typeStyles.text} ${typeStyles.border}`}>
                            {grammar.type}
                        </span>
                        {grammar.level && (
                    ...
                    <h1 className="text-4xl font-black text-foreground mb-4">
                        {i18n.language === 'zh' ? grammar.titleZh || grammar.title :
                            i18n.language === 'vi' ? grammar.titleVi || grammar.title :
                                i18n.language === 'mn' ? grammar.titleMn || grammar.title :
                ...
                <article className="prose prose-slate dark:prose-invert max-w-none">
                    ...
                    <h3 className="text-lg font-black text-foreground mt-8 mb-4 flex items-center gap-2 border-b-2 border-slate-900 dark:border-border pb-2 w-fit">
                        <span className="bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 w-6 h-6 inline-flex items-center justify-center rounded-full text-sm">1</span>
                        {t('grammarDetail.explanation', '详细解析 (Core Explanation)')}
                    </h3>
                        ...
                        {grammar.sections && typeof grammar.sections === 'object' && (
                            <>
                                {('introduction' in grammar.sections && grammar.sections.introduction) && (
                                    <MarkdownRenderer content={getLocalizedSectionText(grammar.sections.introduction)} />
                                )}
                                ...
                                {('comparative' in grammar.sections && grammar.sections.comparative) && (
                                    <div className="mt-6 p-5 bg-orange-50 dark:bg-orange-950/20 border-2 border-orange-200 dark:border-orange-800 rounded-lg shadow-sm">
                                        <h4 className="font-black text-orange-800 dark:text-orange-400 mb-2">💡 {t('grammarDetail.comparative', '易混淆语法比较')}</h4>
                                        <MarkdownRenderer content={getLocalizedSectionText(grammar.sections.comparative)} />
                                    </div>
                                )}
                                {('commonMistakes' in grammar.sections && grammar.sections.commonMistakes) && (
                                    <div className="mt-6 p-5 bg-red-50 dark:bg-red-950/20 border-2 border-red-200 dark:border-red-800 rounded-lg shadow-sm">
                                        <h4 className="font-black text-red-800 dark:text-red-400 mb-2">⚠️ {t('grammarDetail.commonMistakes', '常见使用误区')}</h4>
                                        <MarkdownRenderer content={getLocalizedSectionText(grammar.sections.commonMistakes)} />
                                    </div>
                                )}
                            </>
                        )}
                    ...
                    {hasRules && (
                        <>
                            <h3 className="text-lg font-black text-foreground mt-12 mb-4 flex items-center gap-2 border-b-2 border-slate-900 dark:border-border pb-2 w-fit">
                                <span className="bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 w-6 h-6 inline-flex items-center justify-center rounded-full text-sm">2</span>
                                {t('grammarDetail.rules', '接续规则 (Conjugation Rules)')}
                            </h3>
                            <div className="overflow-x-auto mb-10">
                                <table className="w-full text-left border-collapse border-2 border-slate-900 dark:border-border">
                                    <thead>
                                        <tr className="bg-slate-100 dark:bg-slate-800/50">
                                            <th className="p-3 border-2 border-slate-900 dark:border-border font-black text-foreground">词性 / 结构 (Condition)</th>
                                            <th className="p-3 border-2 border-slate-900 dark:border-border font-black text-foreground">规则变化 (Rule)</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                    ...
                    {listExamples.length > 0 && (
                        <>
                            <h3 className="text-lg font-black text-foreground mt-12 mb-4 flex items-center gap-2 border-b-2 border-slate-900 dark:border-border pb-2 w-fit">
                                <span className="bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 w-6 h-6 inline-flex items-center justify-center rounded-full text-sm">3</span>
                                {t('grammarDetail.examples', '核心例句 (Examples)')}
                            </h3>
                    ...
                    {grammar.quizItems && grammar.quizItems.length > 0 && (
                        <>
                            <h3 className="text-lg font-black text-foreground mt-12 mb-4 flex items-center gap-2 border-b-2 border-slate-900 dark:border-border pb-2 w-fit">
                                <span className="bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 w-6 h-6 inline-flex items-center justify-center rounded-full text-sm">4</span>
                                {t('grammarDetail.quizzes', '实战演练 (Practice Quizzes)')}
                            </h3>
                            <div className="space-y-6 mt-4 mb-4">
                                {grammar.quizItems.map((quiz, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="p-6 bg-indigo-50 dark:bg-indigo-950/20 border-2 border-indigo-200 dark:border-indigo-800 rounded-xl shadow-[4px_4px_0_0_rgba(79,70,229,0.2)]">
                    ...
                    {grammar.customNote && (
                        <div className="mt-12 p-6 bg-slate-100 dark:bg-slate-800/80 border-2 border-slate-900 dark:border-slate-500 rounded-xl shadow-[4px_4px_0_0_#0f172a] dark:shadow-[4px_4px_0_0_#64748b]">
                            <h4 className="font-black text-slate-800 dark:text-slate-200 mb-2 flex items-center gap-2">
                                <span>📝</span> {t('grammarDetail.customNote', '教研组小贴士 (Instructor Note)')}
                            </h4>
