...
const GrammarDirectorySidebar: React.FC<GrammarDirectorySidebarProps> = ({
    ...
    const getUnitName = (unitId: number) => {
        const units: Record<number, string> = {
            1: t('grammarModule.units.unit1', '猜测与推断 (Speculation)'),
            2: t('grammarModule.units.unit2', '对照与转折 (Contrast)'),
            3: t('grammarModule.units.unit3', '原因与理由 (Cause & Reason)'),
            4: t('grammarModule.units.unit4', '目的与意图 (Purpose)'),
            5: t('grammarModule.units.unit5', '进行与完成 (Aspect)'),
            6: t('grammarModule.units.unit6', '状态与其持续 (State)'),
            7: t('grammarModule.units.unit7', '程度与限定 (Degree)'),
            8: t('grammarModule.units.unit8', '假设与假定 (Hypothesis)'),
            9: t('grammarModule.units.unit9', '让步与包含 (Concession)'),
            10: t('grammarModule.units.unit10', '机会与改变 (Opportunity)'),
            11: t('grammarModule.units.unit11', '传闻与引用 (Indirect Speech)'),
            12: t('grammarModule.units.unit12', '必然与经历 (Necessity)'),
            13: t('grammarModule.units.unit13', '罗列与顺序 (Listing)'),
            14: t('grammarModule.units.unit14', '基准与范围 (Standard)'),
            15: t('grammarModule.units.unit15', '助词与添意 (Particles)'),
        };
    ...
    const { unitsMap, unitStats } = useMemo(() => {
        ...
        const filtered = q
            ? courseGrammars.filter(
                g => g.title.toLowerCase().includes(q) || (g.summary && g.summary.toLowerCase().includes(q))
            )
            : courseGrammars;
    ...
    // Auto-expand unit if a grammar inside it is selected
    useEffect(() => {
        if (selectedGrammarId) {
            const grammar = courseGrammars.find(g => g.id === selectedGrammarId);
            if (grammar && !expandedUnits.has(grammar.unitId)) {
                setExpandedUnits(prev => new Set(prev).add(grammar.unitId));
            }
        }
    }, [selectedGrammarId, courseGrammars]);
        ...
        <aside className="w-80 bg-card border-r-2 border-slate-900 dark:border-border shadow-[4px_0px_0px_0px_#0f172a] dark:shadow-[4px_0px_0px_0px_rgba(148,163,184,0.26)] flex flex-col z-20 shrink-0 m-4 rounded-xl overflow-hidden">
            <div className="p-4 border-b-2 border-slate-900 dark:border-border bg-slate-50 dark:bg-slate-900/50">
                <h1 className="font-black text-xl italic tracking-tight text-foreground flex items-center gap-2">
                    <span className="bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 px-1.5 rounded leading-none pt-0.5">H</span>
                    {t('grammarModule.directoryTitle', '语法专项教材')}
                </h1>
                <div className="relative mt-4">
                    <Input
                        type="text"
                        placeholder={t('grammarModule.searchPlaceholder', '搜索语法点...')}
                        value={searchQuery}
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-thin scrollbar-thumb-slate-900 scrollbar-track-transparent dark:scrollbar-thumb-slate-100">
                ...
                {sortedUnitIds.map(unitId => {
                    ...
                    return (
                        <div key={unitId} className="border-2 border-slate-900 dark:border-border rounded-xl overflow-hidden bg-background shadow-[2px_2px_0px_0px_#0f172a] dark:shadow-[2px_2px_0px_0px_rgba(148,163,184,0.26)]">
                            <button
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-2">
                                                    <div className="flex items-center gap-2 min-w-0">
                                                        <span className={`text-[9px] w-4 h-4 flex items-center justify-center rounded-full border shrink-0 ${isActive ? 'bg-white/20 border-white/30' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400'}`}>
                                                            {index + 1}
                                                        </span>
                                                        <span className="truncate">
                                                            {i18n.language === 'zh' ? g.titleZh || g.title :
                                                                i18n.language === 'vi' ? g.titleVi || g.title :
                                                                    i18n.language === 'mn' ? g.titleMn || g.title :
                                                                        g.titleEn || g.title}
                                                        </span>
                                                    </div>
                                                    {isMastered && (
                                                        <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-white' : 'text-slate-900 dark:text-slate-100'}`} />
                                                    )}
                                                </div>
                                            </li>
                                        );
