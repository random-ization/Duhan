...
// Rename to match easier local usage if needed, or just use DTOs
type TopikQuestion = TopikQuestionDto;
// Local exam can have missing _id/createdAt when new
type TopikExam = Partial<TopikExamDto> & {
  id: string;
  ...
  round: number;
  timeLimit: number;
  isPaid: boolean | undefined; // Align with optional in DTO
  questions: TopikQuestion[];
  paperType?: string;
  audioUrl?: string; // Add this explicitly to satisfy interface
};
...
export const TopikManager: React.FC = () => {
  ...
  const updateQuestion = <K extends keyof TopikQuestion>(
    id: number,
    field: K,
  ...
  const handleBulkImageUpload = async (files: FileList) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      console.log(
        `Processing file ${i + 1}/${totalFiles}: ${file.name} -> parsed name: ${fileName}`
      );
    ...
    try {
      ...
      excelData.forEach(row => {
        ...
        if (existingIdx >= 0) {
          ...
          if (score !== undefined) {
            updated.score = parseInt(String(score));
            changed = true;
          }
                  ...
                  <div className="bg-zinc-100 border-l-4 border-zinc-800 p-2 mb-6 font-bold text-zinc-800 text-[17px] font-serif">
                    {currentExam?.type === 'LISTENING' ? (
                      <input
                        ...
                        onChange={e =>
                          updateQuestion(questionsInRange[0].id, 'instruction', e.target.value)
                        }
                            ...
                            <div className="flex-1 space-y-4">
                              ...
                              {qConfig?.uiType === 'ORDERING' && (
                                <div className="border-2 border-zinc-800 p-4 relative rounded-lg">
                                  <span className="absolute -top-3 left-4 bg-white px-2 text-xs font-bold border border-zinc-300 rounded">
                                    보기
                                  </span>
                                  <textarea
                                    ...
                                    value={q.contextBox || ''}
                                    onChange={e =>
                                      updateQuestion(q.id, 'contextBox', e.target.value)
                                    }
                                  />
                                </div>
                              )}
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        >
                                          {img ? (
                                            <>
                                              ...
                                              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/optImg:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                                <div
                                                  className="p-2 bg-white rounded-full text-red-500 hover:bg-red-50 cursor-pointer"
                                                  onClick={e => {
                                                    e.stopPropagation();
                                          ...
                                          <div
                                            className={`absolute top-2 left-2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${q.correctAnswer === optIdx ? 'bg-zinc-900 text-white' : 'bg-zinc-200 text-zinc-600'}`}
                                          >
                                            {optIdx + 1}
                                          </div>
                                        </button>
          ...
          {loading ? (
            ...
            exams.map(exam => (
              <div
                ...
                className={`p-4 rounded-xl cursor-pointer mb-2 transition-all border-2 ${
