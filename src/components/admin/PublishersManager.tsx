...
export const PublishersManager: React.FC = () => {
    ...
    const savePublisher = useMutation(api.publishers.save);
    const generateUploadUrl = useMutation(api.publishers.generateUploadUrl);

    const [editingPub, setEditingPub] = useState<{ id?: string, name: string, imageUrl?: string } | null>(null);
    ...
    if (!publishers || !institutes) return <div className="p-10 text-center"><Loader2 className="animate-spin mx-auto" /></div>;
            ...
            <div className="w-1/3 bg-white border-2 border-zinc-900 rounded-xl p-4 shadow-[4px_4px_0px_0px_#18181B] overflow-y-auto">
                ...
                <div className="space-y-2">
                    {mergedPublishers.map((pub) => (
                        ...
                        >
                            {pub.imageUrl ? (
                                <img src={pub.imageUrl} alt={pub.name} className="w-10 h-10 object-cover rounded border border-zinc-200" />
                            ) : (
            ...
            <div className="flex-1 bg-white border-2 border-zinc-900 rounded-xl p-6 shadow-[4px_4px_0px_0px_#18181B]">
                {editingPub ? (
                    <div className="max-w-md mx-auto space-y-6">
                        ...
                        <div>
                            <label className="block text-xs font-bold mb-1">出版社名称 (必须与教材中的一致)</label>
                            <input
