...
type RecentActivity = { summary: Record<string, number> };

const formatStatValue = (value: number | string): string =>
  typeof value === 'number' ? value.toLocaleString() : value;

const AiCostSummaryCard: React.FC<{ aiUsage: AiUsageStats | undefined }> = ({ aiUsage }) => {
  if (!aiUsage) {
    return (
...
const DailyUsageChartCard: React.FC<{ aiUsage: AiUsageStats | undefined }> = ({ aiUsage }) => {
  const daily = aiUsage?.daily ?? [];
  const maxCalls = daily.length > 0 ? Math.max(...daily.map(d => d.calls), 1) : 1;
...
const LearningActivityCard: React.FC<{ activity: RecentActivity | undefined }> = ({ activity }) => (
  <div className="bg-white border-2 border-zinc-900 rounded-xl p-6 shadow-[4px_4px_0px_0px_#18181B]">
    <div className="flex items-center gap-2 mb-4">
...
export const AdminDashboard: React.FC = () => {
  const overview = useQuery(qRef<NoArgs, OverviewStats>('admin:getOverviewStats'));
  const aiUsage = useQuery(qRef<{ days: number }, AiUsageStats>('admin:getAiUsageStats'), {
    days: 30,
  });
  const activity = useQuery(qRef<{ limit: number }, RecentActivity>('admin:getRecentActivity'), {
    limit: 50,
  });
  ...
  const statCards = [
    ...
    {
      label: '教材数量',
      value: overview?.institutes ?? 0,
      icon: BookOpen,
      color: 'bg-purple-100 text-purple-600',
    },
    {
      label: '词汇量',
      value: overview?.vocabulary ?? 0,
      icon: FileText,
      color: 'bg-green-100 text-green-600',
    },
    {
      label: '语法点',
      value: overview?.grammar ?? 0,
      icon: GraduationCap,
      color: 'bg-orange-100 text-orange-600',
    },
    {
      label: '单元数',
      value: overview?.units ?? 0,
      icon: BarChart3,
      color: 'bg-pink-100 text-pink-600',
    },
    {
      label: 'TOPIK 考试',
      value: overview?.exams ?? 0,
      icon: Activity,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {statCards.map(stat => (
          ...
          >
            ...
            <div className="text-2xl font-black">{formatStatValue(stat.value)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <AiCostSummaryCard aiUsage={aiUsage} />
        <DailyUsageChartCard aiUsage={aiUsage} />
      </div>
