 ...
 * - Default: Use field.
 */
export function getLocalizedContent(data: any, field: string, language: Language = 'zh'): string {
    if (!data) return '';
    ...
    let suffix = '';
    switch (language) {
        case 'en': suffix = 'En'; break;
        case 'vi': suffix = 'Vi'; break;
        case 'mn': suffix = 'Mn'; break;
        case 'zh': suffix = ''; break; // Default field usually
        default: suffix = ''; break;
    }

    // Try specific language field
    const specificValue = data[`${field}${suffix}`];
    if (specificValue && typeof specificValue === 'string' && specificValue.trim() !== '') {
    ...
    if ((!defaultValue || defaultValue.trim() === '') && suffix !== 'En') {
        const enValue = data[`${field}En`];
...
export function getLanguageLabel(lang: Language): string {
    switch (lang) {
        ...
        case 'mn': return 'Монгол'; // Mongolian Cyrillic
        default: return lang;
    }
}
