...
const PUBLISHER_THEMES: Record<string, PublisherTheme> = {
  oer: {
    bg: 'bg-k-butter/50',
    text: 'text-k-ink',
    accent: 'bg-k-butter-deep',
    border: 'border-k-ink',
    light: 'bg-k-butter/20',
  },
  yonsei: {
    bg: 'bg-k-mint/50',
    text: 'text-k-ink',
    accent: 'bg-k-mint-deep',
    border: 'border-k-ink',
    light: 'bg-k-mint/20',
  },
  seoulNational: {
  ...
  topikGrammar: {
    bg: 'bg-k-indigo/50',
    text: 'text-k-bg',
    accent: 'bg-k-indigo-deep',
    border: 'border-k-ink',
    light: 'bg-k-indigo/20',
  },
  default: {
    bg: 'bg-k-card',
    text: 'text-k-ink',
...
export const DesktopCoursesOverviewPage: React.FC = () => {
  ...
  const formatLevel = useCallback(
    (level: string) => {
      ...
      if (/^\d+$/.test(level)) {
        return `${t('common.level', { defaultValue: 'Level' })} ${level}`;
      }
      return level;
    },
    [t]
  ...
  // TOPIK 数据
  const topikHistory = useQuery(qRef<{ limit?: number }, TopikAttempt[]>('topik:getMyHistory'), {
    limit: 5,
  });
  ...
  const currentCourse = useMemo<Course | null>(() => {
    ...
    const recentId =
      recentMaterials?.vocabulary?.instituteId || recentMaterials?.grammar?.instituteId;
    if (recentId) {
  ...
  const effectiveSelectedGrammarId = useMemo(() => {
    ...
    return grammarPoints.some(p => p.id === selectedGrammarId)
      ? selectedGrammarId
      : grammarPoints[0].id;
  ...
  const searchableCourses = useMemo(() => {
    ...
    return courses.map(course => ({
      course,
      searchableText:
        `${course.name} ${course.publisher || ''} ${course.displayLevel || ''}`.toLowerCase(),
    }));
  }, [courses]);

  const filteredCourses = useMemo(() => {
    if (activeTab !== 'mine') return [];
    if (!deferredSearchQuery) return searchableCourses.map(item => item.course);
    return searchableCourses
      .filter(item => item.searchableText.includes(deferredSearchQuery))
      .map(item => item.course);
  }, [activeTab, deferredSearchQuery, searchableCourses]);

  const groupedCourses = useMemo(() => {
    const groups: Record<string, Course[]> = {};
    filteredCourses.forEach(course => {
      const pub = course.publisher || t('coursesLibrary.otherPublisher', { defaultValue: 'Other' });
  ...
  const renderMineTab = () => {
    if (!dashboard)
      return <div className="p-20 text-center text-k-sub font-bold">{t('common.loading')}</div>;
      ...
      <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6">
              <DesktopCard
                ...
                style={{
                  background:
                    'linear-gradient(135deg, rgba(247,232,184,0.3) 0%, rgba(162,59,46,0.05) 100%)',
                }}
              >
                ...
                <div className="p-8 relative z-10 h-full flex flex-col">
                  <div className="flex items-center gap-3 mb-6">
                    <span className="px-2 py-0.5 bg-k-ink text-white text-[10px] font-black rounded uppercase tracking-widest">
                      {t('status.inProgress')} ·{' '}
                      {Math.round((current.completedUnitsCount / (current.totalUnits || 1)) * 100)}%
                    </span>
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[12px] font-bold text-k-ink2/60 uppercase tracking-wide">
                      {current.displayLevel} · {t('common.unit')} {current.currentUnitIndex} /{' '}
                      {current.totalUnits} · {current.totalStudyMinutes} {t('common.minutes')}
                    </div>
                    ...
                    <div className="mt-4 h-1.5 w-full bg-k-ink/10 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(current.completedUnitsCount / (current.totalUnits || 1)) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 mt-8">
                    ...
                    >
                      {t('resumeLearning', { defaultValue: 'Resume' })} →
                    </Button>
                    <Button
                      variant="outline"
                    ...
                    >
                      {t('button.courseSyllabus', { defaultValue: 'Syllabus' })}
                    </Button>
                  </div>
                </div>
              </DesktopCard>
              ...
              <DesktopCard className="p-6">
                <div className="flex items-baseline mb-4">
                  ...
                  <span className="text-[13px] font-black text-k-ink">
                    {t('coursesLibrary.myOtherCourses')}
                  </span>
                  <button
                  ...
                  {enrolledCourses
                    ...
                    .map((c, i) => {
                      const percentage = Math.round(
                        (c.completedUnitsCount / (c.totalUnits || 1)) * 100
                      );
                      return (
                        <div
                          key={i}
                          className="flex gap-4 items-center group cursor-pointer"
                          onClick={() => handleCourseOpen({ id: c.id } as Course)}
                        >
                          <HanjaSeal
                            c={c.name.charAt(0)}
                            size={40}
                            bg={BEAD_TONES[i % BEAD_TONES.length]}
                            round={10}
                            className="shrink-0"
                          />
                          <div className="flex-1">
                            <div className="text-[13px] font-black text-k-ink group-hover:text-k-crimson transition-colors line-clamp-1">
                              {c.name}
                            </div>
            ...
            <DesktopCard className="p-8">
              <div className="flex items-baseline mb-8">
                ...
                <span className="text-[14px] font-black text-k-ink uppercase tracking-wider">
                  {t('coursesOverview.desktop.learningJourney')}
                </span>
                <button
                  onClick={() => navigate(`/course/${current.id}`)}
              ...
              <div className="relative flex justify-between overflow-x-auto pb-4 hide-scrollbar">
                ...
                {journeyUnits.map((unit, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="relative z-10 flex flex-col items-center flex-1 min-w-[120px]"
                    >
                      <div
                        className={cn(
        ...
        <section>
          <div className="flex items-baseline mb-4 px-1">
            ...
            <span className="text-[14px] font-black text-k-ink">
              {t('coursesOverview.desktop.learningTools.title')}
            </span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              {
                k: '詞',
                l: t('coursesOverview.desktop.learningTools.flashcards'),
                s: stats
                  ? t('coursesOverview.desktop.learningTools.reviewStatus', {
                      count: stats.vocabStats?.dueReviews || 0,
                    })
                  : t('common.loading'),
                tone: 'pink',
                path: '/vocab-book',
              ...
              {
                k: '文',
                l: t('coursesOverview.desktop.learningTools.grammar'),
                s: stats
                  ? t('coursesOverview.desktop.learningTools.masteredStatus', {
                      count: stats.grammarStats?.mastered || 0,
                    })
                  : t('common.loading'),
                tone: 'mint',
                path: '/grammar',
              ...
              {
                k: '寫',
                l: t('coursesOverview.desktop.learningTools.typing'),
                s: t('common.challengeMode', { defaultValue: 'Challenge' }),
                tone: 'butter',
                path: '/typing',
              },
              {
                k: '記',
                l: t('coursesOverview.desktop.learningTools.notebook'),
              ...
              <DesktopCard
                key={i}
                className="p-5 hover:border-k-ink/20 transition-all hover:-translate-y-1 cursor-pointer"
        ...
        <section>
          <SectionHead
            kanji="庫"
            title={t('coursesLibrary.title', { defaultValue: 'COURSE LIBRARY' })}
          />
          <div className="space-y-10">
            {groupedCourses.map(([publisher, items]) => {
              const theme =
                PUBLISHER_THEMES[resolvePublisherKey(publisher)] || PUBLISHER_THEMES.default;
              return (
                      ...
                      >
                        <div className={cn('p-4 h-24 flex gap-4 items-center', theme.bg)}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <h4
                              className={cn(
                                'text-[13px] font-black leading-tight mt-1 line-clamp-2',
                                theme.text
                              )}
                            >
                              {course.name}
                        ...
                        <div className="px-4 py-3 flex items-center justify-between bg-white">
                          <div className="text-[10px] font-bold text-k-sub uppercase tracking-wider">
                            {course.totalUnits}{' '}
                            {t('coursesLibrary.units', { defaultValue: 'Units' })}
                          </div>
                          <ChevronRight
                            size={14}
                            className="text-k-sub-light group-hover:text-k-crimson transition-colors"
                          />
                        </div>
                      </DesktopCard>
                    ))}
        ...
        <section>
          <div className="grid grid-cols-3 gap-4">
            <DesktopCard className="p-6 bg-k-crimson/10 border-k-crimson/20 group relative overflow-hidden">
              <div className="absolute -right-6 -bottom-6 font-k-serif text-[120px] font-black opacity-10 group-hover:rotate-12 transition-transform duration-700">
                記
              </div>
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-black mb-2">
                  {t('coursesOverview.desktop.vocabulary.center')}
                </h3>
                ...
                <Button
                  ...
                  className="w-full rounded-xl bg-k-card text-k-ink text-[12px] font-black hover:bg-white transition-colors"
                >
            ...
            <DesktopCard pad={6} className="bg-k-mint/20 border-k-mint-deep/10">
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-black text-k-mint-deep tracking-widest uppercase">
                  Community
                </span>
                <Award size={16} className="text-k-mint-deep" />
              </div>
              <p className="text-[13px] font-bold text-k-ink leading-relaxed">
                {t('coursesOverview.desktop.streakMessage', {
                  count: stats?.streak || 0,
                  percent: Math.min(99, 50 + (stats?.streak || 0) * 2),
                })}
              </p>
            </DesktopCard>
          </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-6 px-1">
            ...
            <div className="relative w-[280px]">
              <Search
                size={16}
                className="absolute left-3.5 top-1/2 -translate-y-1/2 text-k-sub-light"
              />
              <input
                type="text"
              ...
              return (
                <div key={publisher}>
                  <div className="flex items-center gap-3 mb-4 px-2">
                    <div
                      className={cn(
                        'w-8 h-8 rounded-lg flex items-center justify-center border border-k-line/10',
                        theme.bg
                      )}
                    >
                      <span className={cn('text-xs font-black', theme.text)}>
                        {publisher.charAt(0)}
                      </span>
                    </div>
                    <h3 className="text-sm font-black text-k-ink uppercase tracking-widest">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <span className="text-[10px] font-bold text-k-sub-light">
                        ...
                        <ChevronRight
                          size={18}
                          className="text-k-line2 group-hover:text-k-ink transition-colors"
                        />
                      </button>
                    ))}
                  </div>
      ...
      <aside className="space-y-6">
            ...
            {groupedGrammars.map(([unitId, points]) => (
              <div key={unitId}>
                <div className="px-4 py-2 bg-k-bg2/50 text-[10px] font-black text-k-sub uppercase tracking-widest border-y border-k-line/10">
                  {t(`coursesOverview.desktop.grammar.categories.${unitId}`, {
                ...
                {points.map(g => (
                  <div
                    ...
                    className={cn(
                      ...
                      effectiveSelectedGrammarId === g.id
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        effectiveSelectedGrammarId === g.id ? 'text-k-ink' : 'text-k-ink2/80'
                      )}
                    >
                      {g.title}
                    </div>
                    <div className="text-[10px] font-bold text-k-sub mt-1">
                      {g.status === 'MASTERED'
                        ? t('status.mastered', { defaultValue: 'Mastered' })
                        : t('status.learning', { defaultValue: 'Learning' })}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <DesktopCard className="bg-k-ink text-k-bg p-5">
          <div className="text-[10px] font-black tracking-widest uppercase opacity-60 mb-2">
            {t('coursesOverview.desktop.grammar.myProgress', { defaultValue: 'My Progress' })}
          </div>
          <div className="text-2xl font-black mb-1">
            {stats?.grammarStats?.mastered || 0} / {stats?.grammarStats?.total || 0}
          </div>
          <div className="text-[11px] font-bold opacity-70">
            {t('coursesOverview.desktop.grammar.masteryRate', { defaultValue: 'Mastery Rate' })}{' '}
            {stats?.grammarStats?.total
              ? Math.round((stats.grammarStats.mastered / stats.grammarStats.total) * 100)
              : 0}
            %
          </div>
        </DesktopCard>
      ...
      <div className="space-y-6">
        {selectedGrammarDetail ? (
          <DesktopCard className="p-8">
            ...
            <p className="text-[15px] font-bold text-k-sub mb-8">{selectedGrammarDetail.summary}</p>
                ...
                {selectedGrammarDetail.summaryEn && (
                  <RedEyeBlock enabled={redEyeMode}>
                    <div className="text-[12px]" style={{ color: '#bbb' }}>
                      EN: {selectedGrammarDetail.summaryEn}
                    </div>
                  </RedEyeBlock>
                )}
                {selectedGrammarDetail.summaryVi && (
                  <RedEyeBlock enabled={redEyeMode}>
                    <div className="text-[12px]" style={{ color: '#bbb' }}>
                      VI: {selectedGrammarDetail.summaryVi}
                    </div>
                  </RedEyeBlock>
                )}
                {selectedGrammarDetail.summaryMn && (
                  <RedEyeBlock enabled={redEyeMode}>
                    <div className="text-[12px]" style={{ color: '#bbb' }}>
                      MN: {selectedGrammarDetail.summaryMn}
                    </div>
                  </RedEyeBlock>
                )}
              </div>
            ...
            {selectedGrammarDetail.sections && (
              <div className="space-y-[24px] mb-8">
                {selectedGrammarDetail.sections.introduction?.zh && (
                  <div
                    className="rounded-[14px] border px-[24px] py-[18px]"
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.introduction.zh}
                    ...
                    {selectedGrammarDetail.sections.introduction.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.introduction.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
                ...
                {selectedGrammarDetail.sections.core?.zh && (
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.core.zh}
                      redEyeEnabled={redEyeMode}
                      t={t}
                    />
                    {selectedGrammarDetail.sections.core.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.core.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
                ...
                {selectedGrammarDetail.sections.comparative?.zh && (
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.comparative.zh}
                      redEyeEnabled={redEyeMode}
                      t={t}
                    />
                    {selectedGrammarDetail.sections.comparative?.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.comparative.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
                ...
                {selectedGrammarDetail.sections.cultural?.zh && (
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.cultural.zh}
                      redEyeEnabled={redEyeMode}
                      t={t}
                    />
                    {selectedGrammarDetail.sections.cultural?.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.cultural.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
                ...
                {selectedGrammarDetail.sections.commonMistakes?.zh && (
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.commonMistakes.zh}
                      redEyeEnabled={redEyeMode}
                      t={t}
                    />
                    {selectedGrammarDetail.sections.commonMistakes?.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.commonMistakes.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
                ...
                {selectedGrammarDetail.sections.review?.zh && (
                  ...
                  >
                    ...
                    <MarkdownRenderer
                      content={selectedGrammarDetail.sections.review.zh}
                      redEyeEnabled={redEyeMode}
                      t={t}
                    />
                    {selectedGrammarDetail.sections.review?.en && (
                      <RedEyeBlock enabled={redEyeMode}>
                        <div className="mt-6 pt-6 border-t border-dashed border-[#f0ede8]">
                          <MarkdownRenderer
                            content={`EN: ${selectedGrammarDetail.sections.review.en}`}
                            redEyeEnabled={redEyeMode}
                            t={t}
                          />
                        </div>
            ...
            {selectedGrammarDetail.explanation && (
              <div className="mb-8">
                <div
                  className="mb-2 text-[11px] font-extrabold tracking-[2px]"
                  style={{ color: '#999' }}
                >
                  {t('coursesOverview.desktop.grammar.explanation')} · EXPLANATION
                </div>
                <div
                  className="rounded-[14px] px-[24px] py-[18px]"
                  style={{ background: '#faf8f5' }}
                >
                  <MarkdownRenderer
                    content={selectedGrammarDetail.explanation}
                    redEyeEnabled={redEyeMode}
                    t={t}
                  />
                </div>
                {selectedGrammarDetail.explanationEn && (
                  <RedEyeBlock enabled={redEyeMode}>
                    <div
                      className="mt-4 rounded-[14px] px-[24px] py-[18px]"
                      style={{ background: '#faf8f5' }}
                    >
                      <div className="mb-4 text-[11px] font-extrabold uppercase tracking-[0.15em] text-k-sub">
                        ENGLISH EXPLANATION
                      </div>
                      <MarkdownRenderer
                        content={selectedGrammarDetail.explanationEn}
                        redEyeEnabled={redEyeMode}
                        t={t}
                      />
                    </div>
            ...
            {selectedGrammarDetail.conjugationRules && (
              <div className="bg-k-bg2/40 rounded-2xl p-6 border border-k-line/20 mb-8">
                <div className="text-[11px] font-black text-k-sub uppercase tracking-[2px] mb-4">
                  {t('coursesOverview.desktop.grammar.conjugationRules', {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {Object.entries(selectedGrammarDetail.conjugationRules).map(([key, val], idx) => (
                    <div key={idx}>
                      ...
                      <div className="font-k-serif text-2xl text-k-ink">{String(val)}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {selectedGrammarDetail.examples && selectedGrammarDetail.examples.length > 0 && (
              <div className="space-y-3 mb-8">
                <div className="text-[11px] font-black text-k-sub uppercase tracking-[2px] mb-3">
                  {t('coursesOverview.desktop.grammar.examples', { defaultValue: 'EXAMPLES' })}
                </div>
                {selectedGrammarDetail.examples.map((e, i) => (
                  <div
                    key={i}
                    className="group p-5 rounded-xl border border-k-line hover:border-k-ink/10 bg-white transition-all"
                  >
                    <div className="font-k-serif text-lg text-k-ink leading-relaxed">{e.kr}</div>
                    {e.cn && (
                      <RedEyeBlock enabled={redEyeMode}>
          ...
          <div className="flex h-64 flex-col items-center justify-center text-k-sub font-bold gap-4">
            ...
            <div>
              {t('coursesOverview.desktop.grammar.noDetail', {
                defaultValue: 'Grammar detail not available for this language.',
              })}
            </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DesktopCard
          ...
          onClick={() =>
            navigate(
              `/vocab-book/review?${activeCourseId ? `courseId=${activeCourseId}&` : ''}category=DUE`
            )
          }
        >
          <div className="absolute -right-2 -top-4 font-k-serif text-[100px] text-white/10 pointer-events-none">
            復
          </div>
          <div>
        ...
        <DesktopCard
          ...
          onClick={() =>
            navigate(
              `/vocab-book/immerse?${activeCourseId ? `courseId=${activeCourseId}&` : 'all=true&'}category=UNLEARNED`
            )
          }
        >
          <div className="absolute -right-2 -top-4 font-k-serif text-[100px] text-white/10 pointer-events-none">
          ...
          <div>
            <div className="px-2 py-0.5 bg-k-crimson text-white text-[9px] font-black rounded uppercase tracking-widest w-fit mb-2">
              NEW ·{' '}
              {t('coursesOverview.desktop.vocabulary.availableToLearn', {
                defaultValue: 'Available to Learn',
              })}
            </div>
            <div className="text-4xl font-black">
              {stats?.vocabStats?.unlearned || 0}{' '}
              <span className="text-lg opacity-80">
                {t('common.words', { defaultValue: 'Words' })}
              </span>
            </div>
          </div>
          <Button className="w-full bg-k-crimson text-white hover:bg-k-crimson/90 text-[12px] font-black rounded-xl h-10 border-none">
            {t('coursesOverview.desktop.vocabulary.startLearning', {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            k: '學',
            n: stats?.todayWordsStudied || '0',
            l: t('coursesOverview.desktop.vocabulary.studiedToday', {
              defaultValue: 'Studied Today',
            }),
            tone: 'mint',
          },
          {
            k: '通',
            n: stats?.vocabStats?.mastered || '0',
            l: t('coursesOverview.desktop.vocabulary.mastered'),
            tone: 'butter',
          },
          {
            k: '新',
            n: stats?.vocabStats?.unlearned || '0',
            l: t('coursesOverview.desktop.vocabulary.toLearn', { defaultValue: 'To Learn' }),
            tone: 'pink',
          },
          {
            k: '全',
            n: stats?.vocabStats?.total || '0',
            l: t('common.all', { defaultValue: 'All Content' }),
            tone: 'lilac',
          },
        ].map((s, i) => (
        ...
        <div className="lg:col-span-2 space-y-6">
          <section>
            <div className="flex items-baseline mb-4">
              ...
              <span className="text-[14px] font-black text-k-ink uppercase tracking-wider">
                {t('coursesOverview.desktop.vocabulary.myWordbooks', {
                  defaultValue: 'My Wordbooks',
                })}
              </span>
              <button
                className="ml-auto text-[11px] font-bold text-k-sub hover:text-k-crimson"
                onClick={() => navigate('/vocab-book')}
              >
                + {t('coursesOverview.desktop.vocabulary.create', { defaultValue: 'Create' })}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <DesktopCard
                className={cn(
                  ...
                  activeCourseId === null
                    ? 'border-k-crimson shadow-md bg-k-crimson/5'
                    : 'border-transparent'
                )}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-[14px] font-black text-k-ink tracking-tight">
                      {t('common.allWordbooks', { defaultValue: 'All Wordbooks' })}
                    </div>
                    <div className="text-[10px] font-bold text-k-sub mt-0.5">
                      {stats?.vocabStats?.total || 0} {t('words', { defaultValue: 'Words' })}
                    </div>
                  </div>
                </div>
              </DesktopCard>
              ...
              {(() => {
                ...
                return sortedCourses.map((d, i) => {
                  ...
                  const progress = d.totalUnits
                    ? Math.round((d.completedUnits / d.totalUnits) * 100)
                    : 0;
                  ...
                  return (
                    <DesktopCard
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'border-k-crimson shadow-md bg-k-crimson/5'
                          : 'border-transparent hover:border-k-line'
                      )}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <HanjaSeal
                            c={d.courseName.charAt(0)}
                            size={40}
                            bg={`var(--color-k-${tone}-deep)`}
                            round={10}
                          />
                          {progress === 100 && (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            <div className="text-[14px] font-black text-k-ink tracking-tight line-clamp-1">
                              {d.courseName}
                            </div>
                            <div className="text-[10px] font-black text-k-crimson px-1.5 py-0.5 bg-k-crimson/10 rounded-md">
                              {d.displayLevel
                                ? isNaN(Number(d.displayLevel))
                                  ? d.displayLevel
                                  : `${d.displayLevel}${t('common.levelSuffix', { defaultValue: '级' })}`
                                : t('common.book', { defaultValue: 'Book' })}
                            </div>
                          </div>
                          <div className="flex items-center justify-between text-[10px] font-bold text-k-sub">
                            <span>
                              {d.completedUnits} / {d.totalUnits || 0}{' '}
                          ...
                          <div className="mt-2 h-1 w-full bg-k-line rounded-full overflow-hidden">
                            <div
                              className={cn(
                                ...
                                isActive ? 'bg-k-crimson' : `bg-k-${tone}-deep`
                              )}
          ...
          <section>
            <div className="flex items-baseline mb-4">
              ...
              <span className="text-[14px] font-black text-k-ink uppercase tracking-wider">
                {t('coursesOverview.desktop.vocabulary.modes.title', {
                  defaultValue: 'Practice Modes',
                })}
              </span>
            </div>
              ...
              {[
                {
                  id: 'flashcard',
                  k: '閃',
                  l: t('coursesOverview.desktop.vocabulary.modes.flashcard', {
                    defaultValue: '闪卡',
                  }),
                  s: 'SRS',
                },
                {
                  id: 'learn',
                  k: '習',
                  l: t('coursesOverview.desktop.vocabulary.modes.learn', { defaultValue: '学习' }),
                  s: 'Learn',
                },
                {
                  id: 'test',
                  k: '考',
                  l: t('coursesOverview.desktop.vocabulary.modes.test', { defaultValue: '考试' }),
                  s: 'Test',
                },
                {
                  id: 'match',
                  k: '图',
                  l: t('coursesOverview.desktop.vocabulary.modes.match', { defaultValue: '拼图' }),
                  s: 'Match',
                },
              ].map(m => (
                <DesktopCard
        ...
        <div className="space-y-6">
          <DesktopCard className="p-6">
            <div className="flex items-baseline mb-6">
              ...
              <span className="text-[14px] font-black text-k-ink uppercase tracking-wider">
                {t('coursesOverview.desktop.vocabulary.forecast')}
              </span>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-k-line/10 text-center">
              <div className="text-[11px] font-bold text-k-sub tracking-wider uppercase">
                {t('coursesOverview.desktop.vocabulary.totalProgress', {
                  defaultValue: 'Total Progress',
                })}
              </div>
              <div className="text-2xl font-black text-k-ink mt-1">
                {Math.round(
                  ((stats?.vocabStats?.mastered || 0) / (stats?.vocabStats?.total || 1)) * 100
                )}
                %
              </div>
            </div>
          ...
          <DesktopCard className="p-4">
            <div className="text-[11px] font-black text-k-sub uppercase tracking-widest mb-3">
  ...
  const renderTypingTab = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            l: t('coursesOverview.desktop.typing.avgSpeed'),
            v: typingStats?.averageWpm || '0',
            u: 'WPM',
            tone: 'mint',
          },
          {
            l: t('coursesOverview.desktop.typing.accuracy'),
            v: typingStats?.averageAccuracy || '0',
            u: '%',
            tone: 'butter',
          },
          {
            l: t('coursesOverview.desktop.typing.totalTests'),
            v: typingStats?.totalTests || '0',
            u: 'tests',
            tone: 'pink',
          },
          {
            l: t('coursesOverview.desktop.typing.highestSpeed'),
            v: typingStats?.highestWpm || '0',
            u: 'WPM',
            tone: 'lilac',
          },
        ].map((s, i) => (
          <DesktopCard key={i} className="p-5">
            <div className="text-[10px] font-black text-k-sub uppercase tracking-widest mb-2">
              {s.l}
            </div>
      ...
      <DesktopCard className="p-10 relative">
        <div className="absolute top-6 left-8">
          <span className="px-2 py-0.5 bg-k-ink text-white text-[10px] font-black rounded uppercase tracking-widest">
            {sampleTypingText?.category ||
              t('coursesOverview.desktop.typing.commonPhrases', {
                defaultValue: 'Common Phrases',
              })}{' '}
          ...
          {['ㅂ', 'ㅈ', 'ㄷ', 'ㄱ', 'ㅅ', 'ㅛ', 'ㅕ', 'ㅑ', 'ㅐ', 'ㅔ'].map((k, i) => (
            <div
              ...
              className={cn(
                ...
                k === 'ㄱ'
                  ? 'bg-k-crimson text-white scale-110 shadow-lg'
                  : 'bg-white text-k-ink border border-k-line'
              )}
            >
              {k}
            </div>
  ...
  const renderTopikTab = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* 1. Hero Card */}
      <DesktopCard pad={24} className="relative overflow-hidden bg-k-indigo text-white p-8">
        <div className="absolute -right-6 -top-8 font-k-serif text-[160px] text-white/5 pointer-events-none">
          試
        </div>
        <div className="px-2 py-0.5 bg-white/20 text-white text-[10px] font-black rounded uppercase tracking-widest w-fit mb-4">
          TOPIK {featuredTopikExam?.level || 'II'} ·{' '}
          {t('coursesOverview.desktop.topik.mockExam', { defaultValue: 'Mock Exam' })}
        </div>
        <h2 className="text-3xl font-black mb-2 tracking-tight">
          {featuredTopikExam?.title ||
            t('coursesOverview.desktop.topik.examTitlePlaceholder', {
              defaultValue: 'Session 88 · Full Mock Exam',
            })}
        </h2>
        <p className="text-[13px] font-bold opacity-70 mb-8">
          {featuredTopikExam?.description ||
            t('coursesOverview.desktop.topik.examInfo', {
              defaultValue: 'Listening 50 · Writing 4 · Reading 50 · 180 min',
            })}
        </p>
        ...
        <div className="flex items-center gap-12">
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">
              {t('coursesOverview.desktop.topik.predictedScore', {
                defaultValue: 'Predicted Score',
              })}
            </div>
          ...
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">
              {t('coursesOverview.desktop.topik.targetLevel', { defaultValue: 'Target Level' })}
            </div>
            <div className="font-k-serif text-3xl font-medium">
              {featuredTopikExam?.level === 1 ? '2' : '4'}{' '}
              {t('common.levelSuffix', { defaultValue: '级' })}
            </div>
          </div>
          <div className="flex-1" />
          <Button
      ...
      <section>
        <DesktopCard pad={0}>
          <div className="p-5 border-b border-k-line flex items-baseline">
            ...
            <span className="text-[14px] font-black text-k-ink uppercase tracking-wider">
              {t('coursesOverview.desktop.topik.recentAttempts', {
                defaultValue: 'Recent Mock Exam Records',
              })}
            </span>
          </div>
            ...
            {topikHistory && topikHistory.length > 0 ? (
              topikHistory.map(r => {
                ...
                return (
                  <div
                    key={r.id}
                    className="p-5 flex items-center gap-6 hover:bg-k-bg2/30 transition-colors cursor-pointer"
                  >
                    <div className="w-12 h-12 bg-k-bg2 rounded-xl flex flex-col items-center justify-center shrink-0">
                      <div className="text-[9px] font-black text-k-sub uppercase">
                        {dateObj.toLocaleDateString(i18n.language, { month: 'short' })}
                      </div>
                      <div className="text-[18px] font-black text-k-ink leading-none">
                        {dateObj.getDate()}
                      </div>
                    </div>
                    <div className="flex-1">
                    ...
                    <div className="text-right">
                      <div className="font-k-serif text-2xl font-medium text-k-crimson">
                        {r.score}
                      </div>
      ...
      <div className="space-y-6">
        ...
        <DesktopCard className="p-4">
          <div className="flex items-baseline gap-2 mb-3">
            ...
            <span className="text-[11px] font-black text-k-ink uppercase tracking-wider">
              {t('dashboard.desktop.streak')}
            </span>
          </div>
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-[11px] font-bold text-k-sub">
              {t('dashboard.desktop.consecutiveDays')}
            </span>
            ...
            {['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map((dayKey, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-1.5">
                  ...
                  />
                  <span className="text-[9px] font-bold text-k-sub/60">
    ...
    <div className="w-full min-h-screen bg-k-bg pb-20 pt-8">
      <div className="mx-auto w-full max-w-6xl px-6">
        ...
        <div
          className={cn(
            'grid gap-8',
            activeTab === 'mine' ? 'grid-cols-1 lg:grid-cols-[1fr_280px]' : 'grid-cols-1'
          )}
        >
          {/* Main Content */}
          ...
          {/* Right Sidebar */}
          {activeTab === 'mine' && <aside>{renderSidebar()}</aside>}
        </div>
      </div>
    </div>
  );
