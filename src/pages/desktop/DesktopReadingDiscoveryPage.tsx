...
type LibraryUploadBook = EpubLibraryBook & {
  coverImageUrl?: string;
  progress?: number;
};
...
export default function DesktopReadingDiscoveryPage() {
  ...
  const epubs: LibraryUploadBook[] = myUploads || [];
  ...
  const categorizedBooks = useMemo(() => {
    ...
    const groups: Record<string, PublishedReadingBook[]> = {};
    filtered.forEach(book => {
      const level =
        book.levelLabel ||
        t('readingDiscovery.pictureBooks.unclassified', { defaultValue: 'Unclassified' });
  ...
  const nextFeatured = React.useCallback(
    () => setFeaturedIndex(i => (i + 1) % featuredList.length),
    [featuredList.length]
  );
  const prevFeatured = React.useCallback(
  ...
  }, [featuredList.length, nextFeatured]);
  ...
  const tabs: { id: ReadingTab; label: string; icon: LucideIcon }[] = [
    { id: 'all', label: t('readingDiscovery.tabs.all', { defaultValue: 'All' }), icon: Layers },
    {
      id: 'picture_books',
      label: t('readingDiscovery.tabs.pictureBooks', { defaultValue: 'Books' }),
      icon: BookOpen,
    },
    {
      id: 'news',
      label: t('readingDiscovery.tabs.news', { defaultValue: 'News' }),
      icon: Newspaper,
    },
    {
      id: 'epubs',
      label: t('readingDiscovery.tabs.epubs', { defaultValue: 'Library' }),
      icon: BookMarked,
    },
  ];
      ...
      <section className="space-y-6">
        <div className="flex items-baseline justify-between">
          <div className="flex items-baseline">
            ...
            <span className="text-[16px] font-extrabold text-k-ink">
              {t('readingDiscovery.pictureBooks.featured', { defaultValue: 'Featured Books' })}
            </span>
          </div>
          {activeTab === 'all' && (
            <button
            ...
            >
              {t('readingDiscovery.pictureBooks.viewAll', { defaultValue: 'View all' })}{' '}
        ...
        {featured && (
          <div className="relative group/carousel">
            <DesktopCard
              pad={0}
              className="overflow-hidden border border-k-line/10 shadow-k-sh-sm hover:shadow-k-sh-lg transition-all"
            >
              <div className="flex h-[280px]">
                <div className="relative w-[380px] shrink-0 overflow-hidden">
                  ...
                  <div className="absolute left-6 top-6">
                    <DesignChip tone="ink" size="sm" className="backdrop-blur-md bg-k-ink/80">
                      {t('readingDiscovery.pictureBooks.weeklyFeatured', {
                        defaultValue: 'Weekly Featured',
                      })}
                    </DesignChip>
                  </div>
                </div>
                <div className="flex-1 p-10 flex flex-col justify-center animate-in slide-in-from-right-4 duration-500">
                  <div className="flex items-center gap-3 mb-4">
                    <DesignChip tone="butter" size="sm">
                      {featured.levelLabel ||
                        t('readingDiscovery.difficulty.intermediate', {
                          defaultValue: 'Intermediate',
                        })}
                    </DesignChip>
                    <span className="text-[12px] font-bold text-k-sub flex items-center gap-1.5">
                      <Clock size={14} /> {featured.readingMinutes || 5}{' '}
                      {t('readingDiscovery.pictureBooks.readingMinutes', {
                  ...
                  <p className="text-k-sub text-[15px] font-medium mb-8 line-clamp-2 max-w-[500px]">
                    {t('readingDiscovery.pictureBooks.description', {
                      defaultValue:
                        'Explore wonderful Korean stories and improve your reading comprehension.',
                    })}
                  </p>
                  <div className="flex items-center gap-4">
                    ...
                    >
                      {t('readingDiscovery.articles.startReading', {
              ...
              <div className="relative aspect-[4/3] overflow-hidden bg-k-bg2">
                ...
                <div className="absolute right-3 top-3">
                  <DesignChip tone="ink" size="sm" className="backdrop-blur-md bg-k-ink/70">
                    {p.levelLabel ||
                      t('readingDiscovery.meta.reading', { defaultValue: 'Reading' })}
                  </DesignChip>
                </div>
              </div>
              <div className="p-5">
                ...
                <div className="flex items-center gap-3 text-[11px] font-bold text-k-sub">
                  <span>
                    {p.readingMinutes || 5}{' '}
                    {t('readingDiscovery.meta.minutes', { defaultValue: 'min' })}
                  </span>
                  <span className="w-1 h-1 rounded-full bg-k-line" />
                  <span>
                    {p.pageCount || 0} {t('readingDiscovery.meta.pages', { defaultValue: 'pages' })}
                  </span>
                </div>
              </div>
            </DesktopCard>
      ...
      <section className="space-y-8">
        <div className="flex items-baseline justify-between border-b border-k-line pb-4">
          <div className="flex items-baseline gap-3">
            ...
            <span className="text-[20px] font-extrabold text-k-ink">
              {t('readingDiscovery.news.liveTitle', { defaultValue: 'Live News' })}
            </span>
            <span className="ml-2 text-[12px] font-bold text-k-sub bg-k-bg2 px-2 py-0.5 rounded-full">
              {filteredNews.length}{' '}
              {t('readingDiscovery.news.countSuffix', { defaultValue: 'latest updates' })}
            </span>
          </div>
          ...
          {filteredNews.length > 0 ? (
                ...
                >
                  <div className="flex-1 min-w-0 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-bold text-k-sub/60 flex items-center gap-1.5">
                        ...
                        {item.publishedAt
                          ...
                          : t('readingDiscovery.news.justNow', { defaultValue: 'Just now' })}
                      </span>
                    ...
                    <p className="text-[13px] font-medium text-k-sub line-clamp-2 leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">
                      {item.summary ||
                        item.bodyText ||
                        t('readingDiscovery.news.readMorePlaceholder', {
                          defaultValue:
                            'Click to read full article, get more highlights and Korean learning points.',
                        })}
                    </p>
                  ...
                  <div className="shrink-0 w-36 h-36 rounded-2xl overflow-hidden bg-k-bg2 border border-k-line/5 shadow-k-sh-sm group-hover:shadow-k-sh-lg transition-all transform group-hover:scale-[1.02] relative">
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${source.color}08 0%, ${source.color}15 100%)`,
                      }}
                    >
                      <div
            ...
            <div className="col-span-full py-20 flex flex-col items-center justify-center text-k-sub opacity-50 bg-k-bg2/30 rounded-3xl border border-dashed border-k-line">
              ...
              <p className="text-[14px] font-bold">
                {isLoadingNews
                  ? t('readingDiscovery.news.updating', { defaultValue: 'Updating news...' })
                  : t('readingDiscovery.news.empty', { defaultValue: 'No news available' })}
              </p>
            </div>
          )}
        </div>
    ...
    <section className="space-y-6">
      <div className="flex items-baseline justify-between">
        <div className="flex items-baseline">
          ...
          <span className="text-[16px] font-extrabold text-k-ink">
            {t('readingDiscovery.epubs.title', { defaultValue: 'My Library' })}
          </span>
        </div>
        <button
      ...
      {!user?.id ? (
        <div className="py-20 flex flex-col items-center text-center bg-k-bg2/30 rounded-3xl border border-dashed border-k-line">
          ...
          <h3 className="text-[18px] font-bold text-k-ink mb-2">
            {t('readingDiscovery.epubs.privateLibrary', { defaultValue: 'Private Library' })}
          </h3>
          <p className="text-k-sub text-sm mb-6 max-w-[300px]">
            {t('readingDiscovery.epubs.loginPrompt', {
              defaultValue: 'Log in to manage your private library...',
            })}
          </p>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
          {epubs.length > 0 ? (
            epubs.map(book => (
              ...
              >
                ...
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-4 shadow-k-sh-sm group-hover:shadow-k-sh-lg transition-all duration-500 group-hover:-translate-y-2 border border-k-line/5 bg-gradient-to-br from-k-bg2 to-k-line/20">
                  {book.coverSignedUrlCache || book.coverImageUrl ? (
                    <img
                      src={book.coverSignedUrlCache || book.coverImageUrl}
            ...
            <div className="col-span-full py-24 flex flex-col items-center justify-center text-center bg-k-bg2/20 rounded-[40px] border border-dashed border-k-line/40">
              ...
              <h3 className="text-[18px] font-black text-k-ink mb-2">
                {t('readingDiscovery.epubs.empty', { defaultValue: 'Library is empty' })}
              </h3>
              <p className="text-[13px] font-medium text-k-sub mb-8 opacity-60">
                {t('readingDiscovery.epubs.emptyPrompt', {
                  defaultValue: 'Import your first EPUB...',
                })}
              </p>
              <button
                onClick={() => navigate('/reading/upload')}
    ...
    <div className="p-8 space-y-10 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-[28px] font-black text-k-ink tracking-tighter">
            {t('readingDiscovery.title', { defaultValue: 'Immersive Reading' })}
          </h1>
        </div>
      ...
      {(isLoadingBooks || isLoadingNews || isLoadingEpubs) && activeTab === 'all' && (
        <div className="pt-10 pb-20 text-center">
          <div className="inline-flex items-center gap-3 px-6 py-3 bg-k-bg2 rounded-full text-k-sub font-bold animate-pulse">
            ...
            {t('readingDiscovery.news.loadingMore', { defaultValue: 'Loading more content...' })}
          </div>
        </div>
      )}
      {renderAllBooksModal()}
        ...
        <SheetContent className="fixed inset-y-0 right-0 w-[85vw] max-w-[1200px] p-0 bg-k-bg border-l border-k-line overflow-hidden flex flex-col">
          ...
          <div className="shrink-0 h-20 px-8 flex items-center justify-between border-b border-k-line bg-k-card/50 backdrop-blur-md">
            ...
            <div className="relative w-72">
              ...
              <input
                type="text"
                placeholder={t('readingDiscovery.pictureBooks.searchPlaceholder', {
