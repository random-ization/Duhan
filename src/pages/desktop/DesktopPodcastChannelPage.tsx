...
export const DesktopPodcastChannelPage = ({
  ...
  handlePlayEpisode: _handlePlayEpisode,
  onBack,
  handleShareChannel,
}: {
  ...
  return (
    <div
      className={cn(
        'min-h-screen bg-k-bg font-sans transition-all duration-500',
        selectedEpisode ? 'pb-0' : 'pb-32'
      )}
    >
      <div
        className={cn(
          'mx-auto py-12 px-10 transition-all duration-500',
          selectedEpisode ? 'max-w-full' : 'max-w-5xl'
        )}
      >
        <div
          className={cn(
            'grid gap-10',
            selectedEpisode ? 'grid-cols-[1fr_500px] xl:grid-cols-[1fr_600px]' : 'grid-cols-1'
          )}
        >
          <div className="min-w-0">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-k-line pb-4">
                <h2 className="text-[18px] font-extrabold text-k-ink">
                  所有節目{' '}
                  <span className="text-k-sub ml-1 opacity-60">({data?.episodes.length || 0})</span>
                </h2>
              </div>
                ...
                {data?.episodes.map((episode, idx) => (
                  <DesktopCard
                    ...
                    className={cn(
                      ...
                      selectedEpisode?.guid === episode.guid
                        ? 'border-k-crimson bg-k-butter/10'
                        : 'border-transparent hover:border-k-line hover:bg-k-card'
                    )}
                  >
            ...
            {selectedEpisode?.audioUrl && (
              <div className="h-[calc(100vh-100px)] sticky top-12">
                <DesktopCard
                  pad={0}
                  className="h-full overflow-hidden shadow-k-sh-lg border border-k-line/20"
                >
                  <PodcastPlayerModule
                    initialEpisode={{
                      ...selectedEpisode,
