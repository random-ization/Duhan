...
import type { TranscriptSegment } from '../../../convex/transcriptSchema';
...
function getTranscriptTranslation(line: TranscriptSegment | undefined, language: string): string {
  if (!line) return '';
  if (language.startsWith('vi'))
    return line.translationVi ?? line.translationEn ?? line.translation ?? '';
  if (language.startsWith('mn'))
    return line.translationMn ?? line.translationEn ?? line.translation ?? '';
  return line.translationEn ?? line.translation ?? '';
}
  ...
  return (
    <div className="mb-[22px]">
      <div className="mb-2.5 flex items-baseline px-0.5">
        ...
        <span className="text-[11px] font-extrabold tracking-[0.4px] text-k-ink">{title}</span>
        {action && (
...
export default function DesktopVideoPlayerPage() {
  ...
  function formatViews(views?: number): string {
    if (!views) return '0';
    if (i18n.language.startsWith('zh') && views >= 10000)
        ...
        <DesktopCard pad={24}>
          <div className="text-center">
            <div className="text-[18px] font-extrabold text-k-ink">
              {t('coursesOverview.desktop.video.notFound')}
            </div>
            <button
  ...
  const relatedVideos = recommendedVideos?.filter(v => v.id !== video.id).slice(0, 3) || [];

  // 提取视频中的生词（从字幕中获取唯一词汇）
  const vocabFromTranscript =
    transcript.length > 0
      ? transcript.slice(0, 5).map(line => ({
          w: line.text.split(' ').slice(0, 2).join(' '),
          m: line.translation ?? line.translationEn ?? '',
        }))
      : [
  ...
  return (
    <div className="p-6">
      <div className="mb-4 text-[12px] font-bold text-k-sub">
        {t('coursesOverview.desktop.video.title')} ·{' '}
        {video.title || t('coursesOverview.desktop.video.title')}
      </div>
          ...
          <DesktopCard pad={0} className="overflow-hidden">
            <div className="relative aspect-video overflow-hidden bg-k-ink">
              {video.thumbnailUrl ? (
                ...
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(135deg, var(--color-k-indigo) 0%, rgba(162,59,46,0.53) 100%)',
                  }}
                />
              )}
              ...
              <div className="absolute left-[22px] top-[18px]">
                <DesignChip tone="ink" size="sm">
                  ...
                  {video.level || t('common.intermediate', 'Intermediate')}
                </DesignChip>
              </div>
                  ...
                  <div className="absolute bottom-[56px] left-[22px] right-[22px] rounded-[10px] bg-[rgba(0,0,0,0.5)] px-[18px] py-[14px] backdrop-blur-md">
                    ...
                    {globalSettings.mediaSubtitleMode === 'BILINGUAL' ? (
                      <div className="mt-1 text-[12px] font-semibold text-[rgba(255,255,255,0.75)]">
                        {getTranscriptTranslation(transcript[0], i18n.language)}
                      </div>
                    ) : null}
                  </div>
                )}
            ...
            <div className="p-[22px]">
              ...
              <div className="mt-1 text-[12px] font-semibold text-k-sub">
                {t('coursesOverview.desktop.podcast.channel')} ·{' '}
                {video.level || t('common.channel', 'Channel')} · {formatViews(video.views)}{' '}
                {t('coursesOverview.desktop.video.views')}
              </div>
              <div className="mt-3 flex gap-1.5">
                {transcript.length > 0 && (
                  <DesignChip tone="muted" size="sm">
                    📝 {t('coursesOverview.desktop.video.fullTranscript')}
                  </DesignChip>
                )}
                <DesignChip tone="muted" size="sm">
                  📚{' '}
                  {t('coursesOverview.desktop.video.vocabCount', {
                    count: vocabFromTranscript.length,
                  })}
                </DesignChip>
                <DesignChip tone="muted" size="sm">
                  ⏱ {t('coursesOverview.desktop.video.slowPlayback')}
                </DesignChip>
              </div>
            </div>
          </DesktopCard>
        ...
        <div>
          <DRail kanji="次" title={t('coursesOverview.desktop.video.videoVocab')} pad={0}>
            {vocabFromTranscript.map((v, i, a) => (
              ...
              >
                <span className="text-[13px] font-extrabold tracking-[-0.2px] text-k-ink">
                  {v.w}
                </span>
                  ...
                  <div
                    ...
                    style={{
                      background: v.thumbnailUrl
                        ...
                        : ['var(--color-k-pink)', 'var(--color-k-mint)', 'var(--color-k-butter)'][
                            i % 3
                          ],
                    }}
                  />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[12px] font-extrabold text-k-ink">{v.title}</div>
