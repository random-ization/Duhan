...
function getActivityTone(moduleName: string, t: TFunction): ActivityTone {
  ...
  if (lower.includes('vocab') || lower.includes('review')) {
    return { emoji: '📝', tag: t('community.desktop.tags.vocab', { defaultValue: '词汇' }), tone: 'pink', bg: 'var(--color-k-pink)' };
  }
  if (lower.includes('grammar')) {
    return { emoji: '📘', tag: t('community.desktop.tags.grammar', { defaultValue: '语法' }), tone: 'mint', bg: 'var(--color-k-mint-deep)' };
  }
  if (lower.includes('podcast') || lower.includes('listening')) {
    return { emoji: '🎧', tag: t('community.desktop.tags.listening', { defaultValue: '听力' }), tone: 'butter', bg: 'var(--color-k-butter)' };
  }
  if (lower.includes('topik') || lower.includes('exam')) {
    return { emoji: '🎯', tag: t('community.desktop.tags.topik', { defaultValue: 'TOPIK' }), tone: 'lilac', bg: 'var(--color-k-lilac)' };
  }
  if (lower.includes('milestone') || lower.includes('achievement')) {
    return { emoji: '🏆', tag: t('community.desktop.tags.milestone', { defaultValue: '里程碑' }), tone: 'crimson', bg: 'var(--color-k-crimson)' };
  }
  if (lower.includes('qa')) {
    return { emoji: '❓', tag: t('community.desktop.tags.qa', { defaultValue: '问答' }), tone: 'ink', bg: 'var(--color-k-ink)' };
  }
  if (lower.includes('resources')) {
    return { emoji: '📂', tag: t('community.desktop.tags.resources', { defaultValue: '资料' }), tone: 'mint', bg: 'var(--color-k-mint)' };
  }
  return { emoji: '📚', tag: t('community.desktop.tags.study', { defaultValue: '学习' }), tone: 'muted', bg: 'var(--color-k-bg2)' };
}
...
function formatTime(eventAt: number, t: TFunction): string {
  ...
  if (diffMin < 60) return t('dashboard.desktop.minutesAgo', { count: diffMin, defaultValue: `${diffMin}分钟前` });
  const diffHour = Math.floor(diffMin / 60);
  if (diffHour < 24) return t('dashboard.desktop.hoursAgo', { count: diffHour, defaultValue: `${diffHour}小时前` });
...
function formatAction(item: CommunityActivityDto, t: TFunction): string {
  ...
  if (item.eventName === 'review_completed') return (t('dashboard.desktop.reviewCta', { defaultValue: '完成复习' })) + ` ${count}`;
  if (item.eventName === 'session_completed') return (t('dashboard.desktop.approxTime', { count: minutes, defaultValue: `学习了 ${minutes} 分钟` }));
  if (item.eventName === 'content_completed') return t('community.desktop.events.completedContent', { defaultValue: '完成了内容学习' });
  if (item.score !== null) return t('community.desktop.events.topikScore', { score: item.score, defaultValue: `TOPIK 得分: ${item.score}` });
        ...
        {comments?.map(c => (
          ...
          >
            <div className="w-8 h-8 rounded-xl bg-k-bg2 overflow-hidden flex-shrink-0 border border-k-line/5 shadow-sm">
               {c.userAvatar ? <img src={c.userAvatar} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[10px] bg-k-butter/20">👤</div>}
            </div>
            <div className="flex-1 bg-k-bg2/30 rounded-2xl p-3 border border-k-line/5 hover:bg-k-bg2/50 transition-colors">
               <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="text-[10px] font-bold text-k-sub opacity-40 uppercase tracking-tighter">{formatTime(c.createdAt, t)}</div>
               </div>
      ...
      <div className="flex gap-3 mt-5 pt-2">
        <div className="flex-1 relative group">
          <input 
            ...
            placeholder={t('community.desktop.commentPlaceholder', { defaultValue: '写下你的想法...' })}
        ...
        >
          {isSubmitting ? <Loader2 size={16} className="animate-spin" /> : t('community.desktop.reply', { defaultValue: '回复' })}
        </button>
      </div>
    </div>
  );
...
export default function DesktopCommunityPage() {
  ...
  const [friendRelationOverrides, setFriendRelationOverrides] = useState<Record<string, 'already_requested' | 'already_friends' | undefined>>({});
  ...
  const searchUsers = useQuery(FRIENDS.searchUsers, friendSearchQuery.length >= 2 ? { query: friendSearchQuery, limit: 6 } : 'skip');
  ...
  const filters = [
    ...
    { id: 'milestones', label: t('community.desktop.filters.milestones', { defaultValue: '里程碑' }) },
    { id: 'qa', label: t('community.desktop.filters.qa', { defaultValue: '问答' }) },
    { id: 'resources', label: t('community.desktop.filters.resources', { defaultValue: '资料分享' }) },
  ];
  ...
  const feed = useMemo(() => {
    ...
    return communityFeed.map(item => {
      ...
      return {
        ...
        title: item.kind === 'post' ? (item.actorName + ' ' + t('community.desktop.sharedPost', { defaultValue: '分享了动态' })) : formatAction(item, t),
  ...
  const handleToggleLike = async (activityId: string, kind: 'event' | 'post', currentLiked: boolean, currentCount: number) => {
    ...
    try {
      ...
      setLikeState(prev => ({ ...prev, [activityId]: { liked: currentLiked, count: currentCount } }));
    }
  };
          ...
          <div className="flex-1 space-y-4">
            <div className="relative">
              <textarea
                ...
                placeholder={t('community.desktop.composerPlaceholder', { defaultValue: '分享你的学习过程...' })}
                ...
                <div className="flex flex-wrap gap-3 mt-3 px-1">
                  <AnimatePresence>
                    {uploadedImages.map((url) => (
                      ...
                      >
                        <img src={url} className="w-full h-full object-cover rounded-xl border border-k-line/10 shadow-sm" />
                        <button 
            ...
            <motion.div 
              initial={false}
              animate={{ opacity: (postContent || composerFocused || uploadedImages.length > 0) ? 1 : 0.8 }}
            ...
            >
               <div className="flex gap-3">
                  ...
                  >
                    {isUploading ? <Loader2 size={16} className="animate-spin" /> : <ImageIcon size={16} className="text-k-mint-deep" />}
               ...
               >
                 {isPublishing ? <Loader2 size={18} className="animate-spin" /> : <><Send size={16} /> {t('community.desktop.publish', { defaultValue: '发布动态' })}</>}
               </button>
            </motion.div>
          </div>
        </DesktopCard>
      ...
      <AnimatePresence mode="popLayout">
        {!communityFeed ? (
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <DesktopCard pad={60} className="border-dashed border-2 border-k-line/10 bg-transparent flex flex-col items-center">
              ...
              >
                ...
                <div className="flex gap-4 items-center mb-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <span className="text-[16px] font-black text-k-ink hover:text-k-crimson cursor-pointer transition-colors">{p.actorName}</span>
                      <DesignChip tone={p.tone.tone} size="sm">{p.tone.tag}</DesignChip>
                      <div className="flex items-center gap-1.5 ml-auto">
                        <div className="w-1.5 h-1.5 rounded-full bg-k-line/20" />
                        <span className="text-[11px] font-bold text-k-sub/50 uppercase tracking-widest">{p.time}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="pl-1">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      {p.score && (
                        <div className="flex flex-col items-center justify-center w-16 h-16 rounded-2xl bg-k-mint/10 border border-k-mint/20">
                          <div className="text-[10px] font-black text-k-mint-deep/60 uppercase">Score</div>
                    ...
                    <div className="space-y-4">
                      ...
                      {p.images && p.images.length > 0 && (
                        <div className={cn(
                          ...
                          p.images.length === 1 ? "grid-cols-1" : 
                          p.images.length === 2 ? "grid-cols-2" : "grid-cols-2 md:grid-cols-3"
                        )}>
                          {p.images.map((img, idx) => (
                            ...
                            >
                              <img src={img} className="w-full h-full object-cover transition-transform duration-700 group-hover/item:scale-110" />
                  ...
                  {p.attachment && p.attachment.type === 'study_card' && (
                    ...
                    >
                       ...
                       <div className="flex-1">
                          <div className="text-[14px] font-black text-k-ink">{p.attachment.title}</div>
                          <div className="text-[12px] font-bold text-k-sub/70 truncate max-w-[360px]">{p.attachment.description}</div>
                       </div>
                ...
                <div className="flex gap-10 mt-8 pt-6 border-t border-k-line/5">
                  ...
                  >
                    ...
                    {p.likeCount > 0 ? p.likeCount : t('community.desktop.like', { defaultValue: '赞' })}
                  </motion.button>
                  ...
                  >
                    <div className={cn(
                      "w-9 h-9 rounded-full flex items-center justify-center transition-all",
                      expandedPosts.has(p.activityId) ? "bg-k-ink/5" : "bg-transparent group-hover:bg-k-bg2"
                    )}>
                      <MessageCircle size={18} fill={expandedPosts.has(p.activityId) ? "rgba(31,27,23,0.1)" : "none"} />
                    </div>
                    {p.commentCount > 0 ? p.commentCount : t('community.desktop.comment', { defaultValue: '评论' })}
                  </motion.button>
    ...
    <div className="w-[360px] shrink-0 pl-8 space-y-8">
        ...
        <div className="divide-y divide-k-line/5">
          {topFriends.map((p, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className={cn("text-[14px] truncate tracking-tight", p.me ? "font-black text-k-ink" : "font-bold text-k-ink2")}>
                  {p.n}
                </div>
                <div className="text-[10px] font-bold text-k-sub/40 uppercase tracking-widest mt-0.5">Learner</div>
              </div>
              <div className="flex flex-col items-end">
                <span className="font-k-serif text-[16px] text-k-crimson font-black tracking-tighter">
                  {p.xp.toLocaleString()}
                </span>
                <span className="text-[9px] font-black text-k-sub/30 uppercase tracking-tighter">XP</span>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          ...
          {friendSearchQuery.length >= 2 && (
            <div className="space-y-3 pt-1">
               {searchUsers === undefined && <div className="text-center text-[12px] text-k-sub animate-pulse">Searching...</div>}
               {searchUsers?.length === 0 && <div className="text-center text-[12px] text-k-sub">No users found</div>}
               {searchUsers?.map((item: any) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3 bg-k-bg2/30 p-2 rounded-2xl border border-k-line/5">
                       <div className="w-9 h-9 rounded-xl bg-k-card overflow-hidden border border-k-line/10 shrink-0">
                          {item.avatarUrl ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-[12px] font-black text-k-sub/30">{item.name.charAt(0)}</div>
                          )}
                       </div>
                       <div className="flex-1 min-w-0">
                       ...
                       >
                          {relation === 'already_friends' ? '✓' : relation === 'already_requested' ? '...' : '+'}
                       </button>
                    </div>
                  );
               })}
          ...
          {studyPartners.length === 0 ? (
            ...
            studyPartners.map((f, i) => (
              ...
              >
                  ...
                  <div className="w-11 h-11 rounded-[14px] overflow-hidden border-2 border-k-card shadow-sm">
                    {f.avatar ? (
                      ...
                      <div
                        ...
                        style={{ background: ['var(--color-k-pink)', 'var(--color-k-butter)', 'var(--color-k-lilac)'][i % 3] + '30' }}
                      >
                        {f.e}
                      </div>
                    )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[14px] font-black text-k-ink truncate tracking-tight">{f.n}</div>
                  <div className="text-[10px] font-bold text-k-sub/40 uppercase tracking-wider">Active Now</div>
                </div>
              </motion.div>
            ))
          )}
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-[12px] font-black uppercase tracking-[0.2em] text-k-butter/80">社区准则</div>
          </div>
