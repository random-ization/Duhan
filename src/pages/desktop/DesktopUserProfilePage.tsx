...
export default function DesktopUserProfilePage() {
        ...
        <DesktopCard pad={24}>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <DesktopCard pad={16} className="bg-k-bg2/60">
              ...
              <div className="mt-1 text-[22px] font-extrabold text-k-ink">{profile.questionCount}</div>
            </DesktopCard>
            <DesktopCard pad={16} className="bg-k-bg2/60">
              <div className="text-[11px] font-bold uppercase tracking-wider text-k-sub">
                {t('qa.answersGiven', { defaultValue: 'Answers' })}
              </div>
              <div className="mt-1 text-[22px] font-extrabold text-k-ink">{profile.answerCount}</div>
            </DesktopCard>
            <DesktopCard pad={16} className="bg-k-bg2/60">
              <div className="text-[11px] font-bold uppercase tracking-wider text-k-sub">
            ...
            <DesktopCard pad={16} className="bg-k-bg2/60">
              ...
              <div className="mt-1 text-[22px] font-extrabold text-k-ink">{profile.totalVoteScore}</div>
            </DesktopCard>
          </div>
        </DesktopCard>
      </div>
              ...
              profile.recentQuestions.map(question => (
                ...
                >
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-k-sub">{formatRelativeTime(question.createdAt)}</span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center gap-3 text-[11px] text-k-sub">
                    <span>{question.answerCount} {t('qa.answersOne', { defaultValue: 'answers' })}</span>
                    <span>{question.voteScore} {t('qa.totalVotes', { defaultValue: 'votes' })}</span>
                  </div>
                </button>
              ))
            )}
              ...
              profile.recentAnswers.map(answer => (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-3 text-[11px] text-k-sub">
                    ...
                    <span>{answer.voteScore} {t('qa.totalVotes', { defaultValue: 'votes' })}</span>
                    {answer.isAccepted ? (
