...
import { 
  ...
  ArrowLeft, Share2, Heart, Search, MoreHorizontal, ListMusic, RefreshCw, Languages, Repeat
} from 'lucide-react';
      ...
      <div className="flex flex-col h-full space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={cn(
                ...
                isSubscribed ? "text-k-crimson bg-k-crimson/5" : "text-k-sub hover:text-k-crimson hover:bg-k-bg2"
              )}
            >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[360px_1fr] items-stretch gap-8 h-[calc(100vh-220px)] min-h-[400px] overflow-hidden">
          ...
          <div className="space-y-4 h-full overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-k-line/20">
            <DesktopCard pad={0} className="overflow-hidden shadow-k-sh-lg border border-k-line/20">
              <div className="relative aspect-square">
                <img
                  src={episode.channelArtwork || channel.artworkUrl || 'https://placehold.co/800x800'}
                  className="w-full h-full object-cover"
                  alt="cover"
                />
                <div className="absolute bottom-5 left-5 flex gap-2">
                  <DesignChip tone="ink" size="sm" className="backdrop-blur-md bg-k-ink/70 border-white/20">
                    {episode.episodeNumber ? `EP.${episode.episodeNumber}` : 'PODCAST'}
                  </DesignChip>
                  {episode.level && (
                    <DesignChip tone="crimson" size="sm" className="backdrop-blur-md bg-k-crimson/70 border-white/20">
                      {episode.level}
                    </DesignChip>
                  )}
                </div>
            ...
            <DesktopCard pad={24} className="shadow-k-sh border border-k-line/10">
              ...
              >
                <div className="absolute left-0 top-0 h-full bg-k-crimson rounded-full" style={{ width: `${progressPercent}%` }} />
              </div>
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={cn(
                      ...
                      showPlaylist ? "text-k-crimson bg-k-crimson/5" : "text-k-sub hover:bg-k-bg2 hover:text-k-ink"
                    )}
                  >
                    <ListMusic size={18} />
                  </button>
          ...
          <div className="h-full overflow-hidden">
            <DesktopCard pad={0} className="h-full flex flex-col shadow-k-sh border border-k-line/10 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-k-line">
                <div className="flex items-center gap-3">
                  ...
                  {translationStatusLabel && (
                    <DesignChip tone="crimson" size="sm" className="text-[9px] px-1.5 py-0.5 animate-pulse">
                      {translationStatusLabel}
                    </DesignChip>
                  )}
                </div>
                <div className="flex items-center gap-6">
                  {/* Translation Toggle */}
                  <div className="flex items-center gap-3">
                    <span className="text-[11px] font-bold text-k-sub uppercase tracking-wider">{translationLabel}</span>
                    <button
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          isCur ? "text-[20px] font-medium text-k-ink" : "text-[17px] font-medium text-k-ink/60"
                        )}>
                          {/* 
                        ...
                        {(line.translation || isCur) && showTranslation && (
                          <div className={cn(
                            ...
                            isCur ? "text-k-crimson opacity-80" : "text-k-ink/40 group-hover:opacity-100"
                          )}>
        ...
        >
          ...
          >
            <div className="p-6 border-b border-k-line flex items-center justify-between">
              ...
              <button onClick={() => setShowPlaylist(false)} className="text-k-sub hover:text-k-ink">
                關閉
              </button>
            </div>
              ...
              {playlist.map((item, idx) => (
                <button
                  ...
                  className={cn(
                    ...
                    item.guid === episode.guid ? "bg-k-crimson/5 border-k-crimson/20" : "hover:bg-k-bg2"
                  )}
                >
                  <p className={cn(
                  ...
                  <p className="text-[11px] font-bold text-k-sub mt-1">{item.channelTitle || channel.title}</p>
                </button>
              ))}
            </div>
          </div>
