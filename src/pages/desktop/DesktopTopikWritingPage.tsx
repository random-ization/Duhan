...
export default function DesktopTopikWritingPage() {
  ...
  const content = (
    <div className="grid grid-cols-[1fr_320px] gap-[18px]">
          ...
          {activeExam ? (
            <>
              <DesignChip tone="crimson" size="sm">
                {t('coursesOverview.desktop.topikWriting.roundLabel', { count: activeExam.round })}{' '}
                · {activeExam.title}
              </DesignChip>
              <div className="mt-3.5 font-k-serif text-[18px] font-medium leading-[1.6] text-k-ink">
                {activeExam.description ||
                  t('coursesOverview.desktop.topikWriting.defaultDescription')}
              </div>
            </>
          ) : isLoading ? (
            <div className="py-4 text-center text-k-sub font-bold">
              {t('coursesOverview.desktop.topikWriting.fetchingExam')}
            </div>
          ) : (
            <div className="py-4 text-center text-k-sub font-bold">
              {t('coursesOverview.desktop.topikWriting.noChallenge')}
            </div>
          )}
        </DesktopCard>
        ...
        <DesktopCard pad={0} className="min-h-[380px] flex flex-col">
          ...
          >
            <span className="text-[13px] font-extrabold text-k-ink">
              {t('coursesOverview.desktop.topikWriting.answerArea')}
            </span>
            <DesignChip tone="muted" size="sm">
              {t('coursesOverview.desktop.topikWriting.wordCount', { count: wordCount })}
            </DesignChip>
      ...
      <div>
          ...
          <div className="mb-3 grid grid-cols-2 gap-2">
            {[
              ...
              <div key={i} className="rounded-[10px] bg-k-bg2 p-2.5 text-center">
                ...
                <div className="mt-1 font-k-serif text-[18px] font-bold text-k-ink">
                  {m.s || '-'}
                </div>
              </div>
            ))}
        ...
        <DRail
          kanji="考"
          title={t('coursesOverview.desktop.topikWriting.gradingCriteria')}
          pad={16}
        >
  ...
  return (
    <div className="p-6">
      <div className="mb-4 text-[12px] font-bold text-k-sub">TOPIK WRITING</div>
      {content}
    </div>
  );
}
