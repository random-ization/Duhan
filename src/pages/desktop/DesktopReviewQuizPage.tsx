...
import type { VocabReviewItemDto } from '../../../convex/vocab';
...
type ReviewQuizWord = {
  ...
  partOfSpeech?: string;
  example?: string;
  progress?: Parameters<typeof deserializeCard>[0];
};
...
interface DesktopReviewQuizProps {
  modeLabel: string;
  loading: boolean;
  words: ReviewQuizWord[];
  labels: ReviewLabels;
  dueItems: VocabReviewItemDto[];
  mode: string;
  language: string;
    ...
    <div className="grid grid-cols-2 items-start gap-[18px]">
      ...
      <div>
        <div className="mb-4 flex items-center gap-3">
          <DesignChip tone="ink" size="sm">
            {t('coursesOverview.desktop.quiz.reviewing')} · {completedCount} / {dueCount}
          </DesignChip>
          <div
            className="h-1 flex-1 overflow-hidden rounded-full"
            style={{ background: 'var(--color-k-line2)' }}
          >
        ...
        >
          <div className="absolute right-[22px] top-[18px]">
            <DesignChip tone="butter" size="sm">
              {t('coursesOverview.desktop.quiz.reviewNum', { count: 4 })}
            </DesignChip>
          </div>
        ...
        {showAnswer && (
          <div className="mt-[14px] grid grid-cols-4 gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
            {[
              {
                l: t('coursesOverview.desktop.quiz.again'),
                s: t('coursesOverview.desktop.quiz.timeUnit.minute'),
                tone: 'var(--color-k-crimson)',
                kbd: '1',
                quality: 0,
              },
              {
                l: t('coursesOverview.desktop.quiz.hard'),
                s: t('coursesOverview.desktop.quiz.timeUnit.minutes', { count: 6 }),
                tone: 'var(--color-k-pink-deep)',
                kbd: '2',
                quality: 1,
              },
              {
                l: t('coursesOverview.desktop.quiz.good'),
                s: t('coursesOverview.desktop.quiz.timeUnit.day'),
                tone: 'var(--color-k-mint-deep)',
                kbd: '3',
                quality: 3,
              },
              {
                l: t('coursesOverview.desktop.quiz.easy'),
                s: t('coursesOverview.desktop.quiz.timeUnit.days', { count: 5 }),
                tone: 'var(--color-k-indigo)',
                kbd: '4',
                quality: 5,
              },
            ].map((b, i) => (
      ...
      <div>
        {showAnswer ? (
          <DesktopCard
            pad={24}
            className="mb-[14px] animate-in fade-in slide-in-from-right-4 duration-300"
          >
            <DesignChip tone="mint" size="sm">
              {t('coursesOverview.desktop.quiz.revealed')}
            </DesignChip>
            <div className="mt-3 font-k-serif text-[36px] font-medium tracking-[-0.8px] text-k-ink">
              {word.meaningZh || word.meaning}
            </div>
            <div className="mt-1 text-[13px] font-semibold text-k-sub">{word.meaning}</div>
            {word.example && (
          ...
          <DesktopCard
            pad={24}
            className="mb-[14px] flex flex-col items-center justify-center py-12 text-center opacity-40"
          >
            <div className="text-[14px] font-bold text-k-sub">
              {t('coursesOverview.desktop.quiz.pressToReveal', {
                defaultValue: 'Reveal to see meaning',
              })}
            </div>
          </DesktopCard>
        )}
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="text-[12px] font-bold text-k-sub">
          {t('coursesOverview.desktop.review.title').toUpperCase()} ·{' '}
          {t('coursesOverview.desktop.quiz.fsrsReview')}
        </div>
        <button
          type="button"
          onClick={() => navigate(returnActionPath)}
          className="rounded-full border border-k-line bg-k-card px-3 py-1.5 text-[11px] font-black text-k-ink transition hover:border-k-crimson hover:text-k-crimson"
        >
          {returnActionLabel}
        </button>
      </div>
