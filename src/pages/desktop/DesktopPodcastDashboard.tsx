...
import { Play, Library, Search, Disc, History as HistoryIcon, ArrowLeft, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    >
      <div className="relative aspect-square rounded-2xl overflow-hidden mb-3 shadow-k-sh transition-transform duration-500 group-hover:scale-105">
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
          <Play size={32} className="text-white opacity-0 group-hover:opacity-100 transition-all scale-75 group-hover:scale-100 fill-current" />
        </div>
      </div>
      <h4 className="font-extrabold text-[14px] text-k-ink line-clamp-1 group-hover:text-k-crimson transition-colors">
        {channel.title}
      </h4>
      <p className="text-[11px] font-bold text-k-sub mt-0.5 line-clamp-1">
        {channel.author}
      </p>
    </div>
  );
...
export const DesktopPodcastDashboard = ({
  navigate,
  backPath,
  labels,
  podcastMsgs,
  ...
  const handleSelectEpisode = (ep: any, chan?: any) => {
      ...
      <div className="max-w-7xl mx-auto px-10 py-12 space-y-12">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[13px] font-bold text-k-sub mt-0.5">
                {resolveLabel(labels, [['dashboard', 'podcast', 'headerSubtitle']], '韓語播客 · 聽力訓練')}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <form onSubmit={handleSearch} className="relative group">
              ...
              />
              <Search size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-k-sub group-focus-within:text-k-crimson transition-colors" />
            </form>
          </div>
        </div>
          ...
          <section>
            ...
            >
              ...
              <div className="relative w-full md:w-[220px] aspect-square overflow-hidden shrink-0">
                <img
                  src={lastPlayed ? (lastPlayed.channelImage || 'https://placehold.co/400x400') : channelArtwork(featuredChannel)}
            ...
            {subscriptionsLoading ? (
              ...
              <DesktopCard className="text-center py-12 bg-k-bg2/50 border border-dashed border-k-line">
                <p className="text-[14px] font-bold text-k-sub">
                  {resolveLabel(labels, [['dashboard', 'podcast', 'msg', 'EMPTY_SUBSCRIPTIONS']], 'No subscriptions yet')}
                </p>
              </DesktopCard>
            ) : (
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <SectionHead
              kanji="熱"
              title={activeTab === 'community' ? podcastMsgs.DASHBOARD_COMMUNITY : podcastMsgs.DASHBOARD_EDITOR_PICKS}
            />
            <div className="flex gap-1.5 p-1 bg-k-bg2 rounded-xl">
              <button
                ...
                className={cn(
                  ...
                  activeTab === 'community' ? "bg-k-card text-k-crimson shadow-sm" : "text-k-sub hover:text-k-ink"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "px-4 py-1.5 rounded-lg text-[11px] font-extrabold transition-all",
                  activeTab === 'weekly' ? "bg-k-card text-k-crimson shadow-sm" : "text-k-sub hover:text-k-ink"
                )}
              >
            ...
            >
              {displayedTrending.slice(0, 15).map((pod, idx) => (
                <div
                  ...
                  onClick={() => navigate(buildPodcastChannelPath(pod, currentPath), { state: { channel: pod } })}
                ...
                >
                  <div className="relative aspect-square rounded-2xl overflow-hidden mb-3 shadow-k-sh transition-all group-hover:shadow-k-sh-lg">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center">
                      <Play size={36} className="text-white opacity-0 scale-50 group-hover:opacity-100 group-hover:scale-100 transition-all fill-current" />
                    </div>
                  </div>
                  ...
                  >
                    <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 shadow-sm">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                        <Play size={16} className="text-white opacity-0 group-hover:opacity-100 transition-all fill-current" />
                      </div>
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] font-bold text-k-sub truncate">
                        {record.channelName} • {formatSafeDateLabel(
                          record.playedAt,
...
export default DesktopPodcastDashboard;
