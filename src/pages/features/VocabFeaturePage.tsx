...
import { PageShell, Card, HanjaSeal } from '../../components/mobile/ksoft/ksoft';
      ...
      <div className="pt-32 pb-40">
        ...
        <header className="max-w-7xl mx-auto px-6 mb-24 text-center">
          ...
          >
            ...
            <span className="text-[11px] font-black text-k-ink uppercase tracking-[0.2em]">
              FSRS Algorithm Driven
            </span>
          </motion.div>
          ...
          >
            不再遗忘，
            <br className="hidden md:block" />用{' '}
            <span className="text-k-crimson italic font-k-serif">科学</span> 驯服每一个单词
          </motion.h1>
          ...
          >
            K-Soft 采用最先进的 FSRS
            记忆算法。它能像人类大脑一样理解你的遗忘规律，在最关键的时刻提醒你复习，让记忆效率提升
            300% 以上。
          </motion.p>
        </header>
          ...
          <div className="bg-k-ink rounded-[48px] p-12 md:p-20 text-white overflow-hidden relative">
            ...
            <div className="relative z-10 grid md:grid-cols-2 gap-20 items-center">
              <div>
                ...
                <h2 className="text-[36px] md:text-[48px] font-black mb-8 leading-tight">
                  FSRS 算法：
                  <br />
                  懂你的遗忘，更懂你的努力
                </h2>
                <p className="text-[18px] opacity-60 leading-relaxed mb-10">
                  传统的艾宾浩斯曲线已经过时了。FSRS
                  算法根据你的每一次反馈动态调整复习间隔。对于简单的词，它会果断推迟；对于难词，它会精准覆盖。
                </p>
              ...
              <div className="relative">
                <Card tone="white" className="p-8 text-k-ink shadow-2xl">
                  <div className="flex justify-between items-end mb-8">
                    <div>
                      <div className="text-[11px] font-black text-k-sub uppercase mb-1">
                        Current Word
                      </div>
                  ...
                  <div className="h-32 flex items-end gap-2 mb-6">
                    {[30, 45, 60, 40, 75, 82].map((h, i) => (
                      ...
                      >
                        <div
                          className={`absolute inset-0 rounded-t-lg transition-opacity ${i === 5 ? 'bg-k-crimson' : 'bg-k-ink opacity-20'}`}
                        />
                      </motion.div>
                    ))}
                  </div>
        ...
        <section className="text-center mt-32">
          <h2 className="text-[32px] md:text-[56px] font-black text-k-ink mb-10">
            开始建立你的终身词库
          </h2>
          <button
...
const ModeCard: React.FC<{
  ...
  color: string;
  dark?: boolean;
}> = ({ icon, title, desc, color, dark }) => (
  <Card
    tone={color}
    className={`p-8 h-full border border-k-ink/5 transition-transform hover:-translate-y-2 ${dark ? 'text-white' : 'text-k-ink'}`}
  >
    <div
      className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 ${dark ? 'bg-white/10' : 'bg-k-ink/5'}`}
    >
      {icon}
    </div>
    <h4 className="text-[20px] font-black mb-3">{title}</h4>
    <p className={`text-[14px] font-medium leading-relaxed ${dark ? 'opacity-60' : 'opacity-70'}`}>
      {desc}
    </p>
  </Card>
);
