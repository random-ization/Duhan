...
import { Sparkles, Brain, Target, Clock, BookOpen, FileText, ShieldCheck } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { useLocalizedNavigate } from '@/hooks/useLocalizedNavigate';

const DesktopPracticeHubPage = lazy(() => import('./desktop/DesktopPracticeHubPage'));
import { useQuery } from 'convex/react';
import { SENTENCE_EXPLAINER, VOCAB, qRef, NoArgs } from '@/utils/convexRefs';
...
export default function PracticeHubPage() {
  ...
  const reviewAggregate =
    useQuery(qRef<NoArgs, ReviewAggregate>('learningAssets:getReviewAggregate')) || null;
  const qualityQueue =
    useQuery(SENTENCE_EXPLAINER.getQualityReviewQueue, {
      limit: 20,
      maxConfidence: 0.82,
    }) || [];
  ...
  const reviewSidebarContent = (
    <div className="space-y-3">
      ...
      <ContextualSection title={t('reviewPage.modes.title', { defaultValue: 'Choose a Mode' })}>
        ...
        <ContextualListItemButton
          icon={FileText}
          label={t('reviewPage.modes.sentence.title', { defaultValue: 'Sentence Review' })}
          subtitle={t('reviewPage.modes.sentence.desc', {
            defaultValue: 'Review saved sentences in context',
          })}
          trailing={
            <ContextualCountBadge value={reviewAggregate?.savedSentenceCount ?? 0} tone="accent" />
          }
          onClick={() => navigate('/review/quiz?mode=sentences')}
        />
        <ContextualListItemButton
          icon={BookOpen}
          label={t('reviewPage.modes.grammar.title', { defaultValue: 'Grammar Review' })}
          subtitle={t('reviewPage.modes.grammar.desc', {
            defaultValue: 'Practice saved grammar patterns',
          })}
          trailing={
            <ContextualCountBadge value={reviewAggregate?.savedGrammarCount ?? 0} tone="accent" />
          }
          onClick={() => navigate('/review/quiz?mode=grammar')}
        />
        <ContextualListItemButton
          icon={ShieldCheck}
          label={t('reviewPage.quality.title', { defaultValue: 'AI Asset Quality' })}
          subtitle={t('reviewPage.quality.desc', {
            defaultValue: 'Approve or reject low-confidence assets',
          })}
          trailing={<ContextualCountBadge value={qualityQueue.length} tone="warning" />}
          onClick={() => navigate('/learning/asset-quality')}
        />
      </ContextualSection>
  ...
  if (isMobile) {
    const modeCards = [
      ...
      },
      {
        seal: '文',
        tone: KT.sky,
        title: t('reviewPage.modes.sentence.title', { defaultValue: 'Sentence Review' }),
        desc: t('reviewPage.modes.sentence.desc', {
          defaultValue: 'Review saved sentences in context',
        }),
        meta: `${reviewAggregate?.savedSentenceCount ?? 0}`,
        path: '/review/quiz?mode=sentences',
      },
      {
        seal: '審',
        tone: KT.butter,
        title: t('reviewPage.quality.title', { defaultValue: 'AI Asset Quality' }),
        desc: t('reviewPage.quality.desc', {
          defaultValue: 'Approve or reject low-confidence assets',
        }),
        meta: `${qualityQueue.length}`,
        path: '/learning/asset-quality',
      },
      {
        seal: '法',
        tone: KT.lilac,
        title: t('reviewPage.modes.grammar.title', { defaultValue: 'Grammar Review' }),
        desc: t('reviewPage.modes.grammar.desc', {
          defaultValue: 'Practice saved grammar patterns',
        }),
        meta: `${reviewAggregate?.savedGrammarCount ?? 0}`,
