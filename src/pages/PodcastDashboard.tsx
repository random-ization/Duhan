...
const DesktopPodcastDashboard = lazy(() => import('./desktop/DesktopPodcastDashboard'));
  ...
  if (isMobile) {
    return (
      <MobilePodcastDashboard
        view={location.pathname.endsWith('/subscriptions') ? 'subscriptions' : 'home'}
      />
    );
  }

  return (
    <Suspense fallback={<div className="flex items-center justify-center h-screen bg-background text-muted-foreground font-sans">Loading...</div>}>
