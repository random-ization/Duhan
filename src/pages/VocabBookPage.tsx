...
import { Search, BookOpen, Loader2, ArrowLeft, Clock, Zap, Flame, Star, TrendingUp } from 'lucide-react';
...
const VocabBookPage: React.FC = () => {
    ...
    const formatRelativeTime = (timestamp: number | null) => {
        ...
        if (hours > 0) return (labels.vocab?.hoursLater || "{count}h").replace('{count}', String(hours));
    ...
    const filterButtons = [
        ...
        { key: 'LEARNING', label: labels.vocab?.filterLearning || 'Learning', count: stats.learning, color: 'amber' },
        { key: 'REVIEW', label: labels.vocab?.filterReview || 'Review', count: stats.review, color: 'purple' },
    ];
                    ...
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">{labels.dashboard?.vocab?.title || "Vocab Book"}</h1>
                                    <p className="text-slate-500 font-bold text-sm">{labels.dashboard?.vocab?.subtitle || "SRS Smart Review"}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {stats.dueNow > 0 && (
                            <div className="hidden sm:flex items-center gap-3 px-5 py-3 bg-gradient-to-r from-red-50 to-orange-50 rounded-2xl border-[3px] border-red-200 shadow-[0_4px_15px_rgba(239,68,68,0.15),inset_0_2px_4px_rgba(255,255,255,0.9)]">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-bold text-red-400">{labels.dashboard?.vocab?.dueNow || "Due Now"}</p>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {filterButtons.map(btn => (
                                <button
                                    ...
                                    className={`px-4 py-2.5 rounded-xl font-bold text-sm border-[3px] transition-all duration-200 ${activeFilter === btn.key
                                            ? btn.color === 'blue' ? 'bg-blue-500 text-white border-blue-400 shadow-[0_4px_12px_rgba(59,130,246,0.3)]'
                                                : btn.color === 'amber' ? 'bg-amber-500 text-white border-amber-400 shadow-[0_4px_12px_rgba(245,158,11,0.3)]'
                                                    : btn.color === 'purple' ? 'bg-purple-500 text-white border-purple-400 shadow-[0_4px_12px_rgba(139,92,246,0.3)]'
                                ...
                                >
                                    {btn.label}
                                    <span className={`ml-2 px-2 py-0.5 rounded-lg text-xs ${activeFilter === btn.key ? 'bg-white/20' : 'bg-slate-100'
                                        }`}>{btn.count}</span>
                                </button>
                            ))}
                        </div>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <p className="text-xl font-black text-slate-700 mb-2">
                            {searchQuery ? (labels.dashboard?.vocab?.noMatch || 'No results found') : (labels.dashboard?.vocab?.noDueNow || 'No words due for review')}
                        </p>
                        <p className="text-slate-400 font-medium text-center max-w-md">
                            {searchQuery ? '' : (labels.dashboard?.vocab?.srsDesc || "Words you mark as 'Don't know' will appear here for spaced repetition learning")}
                        </p>
                    </motion.div>
                ) : (
                    <motion.div
                            ...
                            {filteredWords.map((word, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Status Badge */}
                                        {statusConfig && (
                                            <div className={`absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 rounded-xl ${statusConfig.bgClass} border-2 ${statusConfig.borderClass}`}>
                                                <StatusIcon className={`w-3.5 h-3.5 ${statusConfig.textClass}`} />
                                                <span className={`text-xs font-black ${statusConfig.textClass}`}>{statusConfig.label}</span>
                                            </div>
                                        )}

                                        {/* Word & Meaning */}
                                        <div className="pr-24">
                                            <h3 className="text-2xl font-black text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors">{word.word}</h3>
                                            {word.pronunciation && (
                                                <p className="text-sm text-slate-400 font-medium mb-2">{word.pronunciation}</p>
                                            )}
                                        ...
                                        <div className="flex items-center justify-between mt-4 pt-4 border-t-2 border-dashed border-slate-100">
                                            <div className="flex items-center gap-3">
                                                {word.partOfSpeech && (
                                                    <span className="px-2.5 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-500">{word.partOfSpeech}</span>
                                                )}
                                                {word.hanja && (
                                            ...
                                            <div className="flex items-center gap-3 text-sm">
                                                <div className="flex items-center gap-1 text-slate-400">
                                                    ...
                                                    <span className="font-bold">{formatRelativeTime(word.progress.nextReviewAt)}</span>
                                                </div>
