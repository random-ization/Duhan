...
import {
  ...
  Trophy, CheckCircle, XCircle, Crown, Mail, Settings
} from 'lucide-react';
        ...
        <div className="text-center md:text-left flex-1">
          ...
          <div className="mt-4 flex flex-wrap gap-4 justify-center md:justify-start">
            <div className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
              ...
              {labels.profile?.joined || 'Joined'} {new Date(user.createdAt || 0).toLocaleDateString()}
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-4">
              ...
              <div className="pt-4">
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-xl font-bold text-white shadow-lg transition-all ${!currentPassword || !newPassword || !confirmPassword || isChangingPassword
                    ? 'bg-slate-300 cursor-not-allowed'
                    : 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:scale-[1.02] active:scale-[0.98]'
                    }`}
                >
                  {isChangingPassword ? <Loading size="sm" /> : (labels.profile?.updatePassword || 'Update Password')}
                </button>
              </div>
            </form>
