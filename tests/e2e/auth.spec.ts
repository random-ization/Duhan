...
test.describe('Authentication Flow', () => {
    test('should display login page correctly', async ({ page }) => {
        ...
        // Check page elements
        await expect(page.locator('h1')).toContainText(/DuHan|登录/);
    ...
    test('should navigate to forgot password page', async ({ page }) => {
        ...
        const forgotLink = page.locator('a:has-text("忘记密码"), a:has-text("Forgot"), a[href*="forgot"]');
...
test.describe('Landing Page', () => {
    test('should load landing page', async ({ page }) => {
        ...
        // Use .first() since there are multiple logo images (header + footer)
    ...
    test('should have navigation links', async ({ page }) => {
        ...
        const ctaButton = page.locator([
            'a:has-text("登录")', 'a:has-text("Log in")',
            'a:has-text("开始使用")', 'a:has-text("Get Started")',
            'a:has-text("免费开始学习")', 'a:has-text("Start Free Trial")',
            'a:has-text("查看课程体系")', 'a:has-text("View Curriculum")',
