...
export const exportVocabBookPdf = action({
  ...
  handler: async (ctx, args) => {
    ...
    const s = {
      ...
      header: { flexDirection: 'row' as const, justifyContent: 'space-between' as const, marginBottom: 12 },
      title: { fontSize: 16, fontFamily: baseFont, fontWeight: 700, color: '#0f172a' },
      subtitle: { marginTop: 4, fontSize: 9, color: '#64748b', fontWeight: 700, fontFamily: baseFont },
      qrWrap: { alignItems: 'flex-end' as const },
      qr: { width: 52, height: 52 },
      domain: { marginTop: 4, fontSize: 8, color: '#334155', fontWeight: 700, fontFamily: baseFont },
      ...
      cellNo: { width: 24, paddingVertical: 5, paddingHorizontal: 6, borderRightWidth: 1, borderRightColor: '#e2e8f0', justifyContent: 'center' as const, alignItems: 'center' as const },
      cellWord: { flex: 1, paddingVertical: 5, paddingHorizontal: 6, borderRightWidth: 1, borderRightColor: '#e2e8f0' },
      cellMeaning: { flex: 1.2, paddingVertical: 5, paddingHorizontal: 6, borderRightWidth: 1, borderRightColor: '#e2e8f0' },
      cellCheck: { width: 18, paddingVertical: 5, paddingHorizontal: 6, justifyContent: 'center' as const, alignItems: 'center' as const },
      headText: { fontSize: 9, color: '#ffffff', fontWeight: 700, textAlign: 'center' as const, fontFamily: baseFont },
      ...
      footer: { position: 'absolute' as const, left: 28, right: 28, bottom: 20, flexDirection: 'row' as const, alignItems: 'center' as const },
    ...
    const Doc = (
      <Document>
        {Array.from({ length: pageCount }).map((_, pageIndex) => {
          ...
          const rightItems = isFoldDictation ? leftItems : pageItems.slice(rowsPerCol, rowsPerCol * 2);

          const renderTable = (half: 'left' | 'right', items: typeof filtered, baseNo: number, styleVariant: 'left' | 'right') => {
            ...
            const paddedItems = Array.from({ length: rowsPerCol }).map((_, idx) => {
              ...
              return { id: `empty-${baseNo}-${idx}`, word: '', meaning: '', type: 'empty', displayIndex: baseNo + idx };
            });

            return (
              <View style={styleVariant === 'left' ? s.tableLeft : s.table}>
                <View style={s.rowHead}>
                  <View style={s.cellNo}><Text style={s.headText}>#</Text></View>
                  <View style={s.cellWord}><Text style={s.headText}>{labels.wordHeader}</Text></View>
                  <View style={s.cellMeaning}><Text style={s.headText}>{labels.meaningHeader}</Text></View>
                  <View style={s.cellCheck}><Text style={s.headText}> </Text></View>
                </View>
                {paddedItems.map((it) => {
                  ...
                  return (
                    <View key={it.id} style={isAlt ? [s.row, s.alt] : s.row}>
                      <View style={s.cellNo}>
                        <Text style={s.noText}>{it.type === 'data' ? String(it.displayIndex) : ''}</Text>
                      </View>
                      ...
                      <View style={s.cellCheck}><View style={s.checkBox} /></View>
                    </View>
                  );
                })}
              </View>
