...
export const checkIntegrity = query({
    ...
    handler: async (ctx) => {
        ...
        for (const course of courses) {
            // Calculate total expected units based on levels/units config
            ...
            for (const unitId of expectedUnits) {
                ...
                if (count === 0) {
                    ...
                    const offsetCount = await ctx.db
                        .query('course_grammars')
                        .withIndex('by_course_unit', q => q.eq('courseId', course.id).eq('unitId', storageUnitId + 10))
                        .collect()
                        .then(res => res.length);

                    if (offsetCount > 0) {
                        // It exists but at offset
                        // missingUnits.push(unitId + " (Found at +10)"); // Type error if number array
                        // Let's just track it differently or report it
