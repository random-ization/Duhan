...
export const migrateVocabNotebooks = mutation({
  ...
  handler: async ctx => {
    ...
    const vocabNotebooks = await ctx.db
      .query('notebooks')
      .filter(q => q.and(q.eq(q.field('userId'), userId), q.eq(q.field('type'), 'VOCAB')))
      .collect();
    ...
    for (const notebook of vocabNotebooks) {
      try {
        ...
        const content: unknown = notebook.content;
        const meaning =
          readString(content, ['meaning']) ??
        ...
        errors.push(`${notebook.title}: ${toErrorMessage(e)}`);
      }
    }
