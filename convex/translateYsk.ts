...
type TranslateYskInternalRefs = {
  getItemsToTranslate: FunctionReference<
    ...
    TranslationItem[]
  >;
  getItemsToReview: FunctionReference<'query', 'internal', Record<string, never>, ReviewItem[]>;
...
const translateYskInternal: TranslateYskInternalRefs = (
  internal as unknown as { translateYsk: TranslateYskInternalRefs }
).translateYsk;
...
function shouldTranslateGrammar(item: GrammarLocalizationItem): boolean {
  ...
  const hasMissingExampleLocales = item.examples.some(
    ex => isMissingLocale(ex.cn) || isMissingLocale(ex.vi) || isMissingLocale(ex.mn)
  );
...
export const auditCourseLocalization = query({
  ...
  handler: async (ctx, args) => {
    ...
    const grammarMissing = grammars.filter(Boolean).filter(grammar =>
      shouldTranslateGrammar({
        id: grammar!._id,
        unitIds: [],
        title: grammar!.title,
        summary: grammar!.summary,
...
Return strict JSON:
{
  ...
  "exampleMn":["..."]
      ...
      try {
        ...
        const summaryEnFallback =
          item.summaryEn || (!hasHanCharacters(item.summary) ? item.summary : undefined);
        const explanationEnFallback =
          item.explanationEn ||
          (!hasHanCharacters(item.explanation) ? item.explanation : undefined);
...
export const runFix = action({
  ...
  handler: async (ctx, args) => {
    const items = await ctx.runQuery(translateYskInternal.getItemsToReview);
...
`;
    ...
    try {
      ...
      if (resultJson) {
        ...
        if (parsed.translations && Array.isArray(parsed.translations)) {
          for (const t of parsed.translations) {
            const originalItem = chunk.find(
              (candidate: ReviewItem) => candidate.appearanceId === t.id
            );
            if (originalItem) {
              updates.push({
                appearanceId: originalItem.appearanceId,
...
export const getBadTranslations = query({
  ...
  handler: async ctx => {
    ...
    for (const app of appearances) {
      ...
      if (word) {
        const isBadZh =
          app.meaning && (app.meaning.length > 20 || /[a-zA-Z1-9。]/.test(app.meaning));
        const isBadMn =
          app.meaningMn &&
          (app.meaningMn.length > 30 ||
            /[a-zA-Z1-9]/.test(app.meaningMn) ||
            app.meaningMn.endsWith('.'));
        const isBadVi =
          app.meaningVi &&
          (app.meaningVi.length > 30 || /[1-9]/.test(app.meaningVi) || app.meaningVi.endsWith('.'));
...
export const updateUnitTranslations = mutation({
  ...
  handler: async (ctx, args) => {
    for (const update of args.updates) {
      ...
      if (transcriptData) {
        const transcriptWrite = await replaceTextbookUnitTranscriptChunks(ctx, id, transcriptData);
...
export const backfillListeningTranslations = action({
  ...
  handler: async (ctx, args) => {
    ...
    const units = allUnits
      .filter(unit =>
        typeof args.unitIndex === 'number' ? unit.unitIndex === args.unitIndex : true
      )
      .sort((a, b) => a.unitIndex - b.unitIndex || a.articleIndex - b.articleIndex);
