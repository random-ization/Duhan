...
type NoteMetadata = NonNullable<NotePageDoc['metadata']>;
type NoteMetadataValue = NoteMetadata[string];
type NoteBlockContent = Doc<'note_blocks'>['content'];
type NoteSourceRef = NonNullable<Doc<'note_review_queue'>['sourceRef']>;
...
const normalizeObjectLeaf = (value: unknown): NoteMetadataValue | undefined => {
...
const normalizeMetadata = (raw: unknown): NoteMetadata => {
  const input = asRecord(raw) || {};
  const metadata: NoteMetadata = {};
  for (const [key, value] of Object.entries(input)) {
    const normalized = normalizeObjectLeaf(value);
...
const normalizeSourceRef = (raw: unknown): NoteSourceRef => {
  const normalized = normalizeObjectLeaf(raw);
  return normalized && !Array.isArray(normalized) && typeof normalized === 'object'
    ? normalized
    : {};
};

const normalizeBlockContent = (value: unknown): NoteBlockContent => {
  try {
    const serialized = JSON.stringify(value);
    if (serialized === undefined) return '';
    return JSON.parse(serialized) as NoteBlockContent;
  } catch {
    return '';
  }
};
  ...
  handler: async (ctx, args) => {
    ...
    if (existingQueue.length > 0) {
      ...
      await ctx.db.insert('note_review_queue', {
        ...
        status: 'queued',
        scheduledFor,
        sourceRef: normalizeSourceRef(metadata.sourceRef),
        createdAt: now,
        updatedAt: now,
      });
    }
...
export const saveEditorDoc = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert('note_blocks', {
      ...
      content: normalizeBlockContent(doc),
      props: {},
      sortOrder: 0,
      createdAt: now,
      updatedAt: now,
  ...
  if (args.blocks) {
    ...
    for (let index = 0; index < sorted.length; index += 1) {
      ...
      await ctx.db.insert('note_blocks', {
        ...
        props: normalizeMetadata(block.props),
        sortOrder: index,
        createdAt: now,
        updatedAt: now,
      });
    ...
    if (quoteText) {
      ...
      if (quoteBlock) {
        await ctx.db.patch(quoteBlock._id, {
          ...
          props: normalizeMetadata({ ...(quoteBlock.props || {}), sourceRef }),
          sortOrder: 0,
          updatedAt: now,
        });
      } else {
        await ctx.db.insert('note_blocks', {
          ...
          blockType: 'quote',
          content: quoteContent,
          props: normalizeMetadata({ sourceRef }),
          sortOrder: 0,
          createdAt: now,
          updatedAt: now,
        });
