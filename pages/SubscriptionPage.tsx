            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-lg shadow-indigo-100/50 dark:shadow-none hover:-translate-y-1 transition-transform">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">{t('coursesOverview.feature1Title')}</h3>
                        <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                            <Trans i18nKey="coursesOverview.feature1Desc">
                                <span className="font-bold text-indigo-600">Premium Members</span> unlock full digital textbooks...
                            </Trans>
                        </p>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-lg shadow-indigo-100/50 dark:shadow-none hover:-translate-y-1 transition-transform">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">{t('coursesOverview.feature3Title')}</h3>
                        <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-100 dark:border-slate-600">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('coursesOverview.booksDigitized')}</span>
                ...
                <div className="mb-24 overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700">
                    <div className="bg-slate-50 dark:bg-slate-800 p-6 border-b border-slate-200 dark:border-slate-700 text-center">
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{t('coursesOverview.benefitComparison')}</h3>
                        <p className="text-slate-500 dark:text-slate-400 mt-2">{t('coursesOverview.seeDifference')}</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full bg-white dark:bg-slate-900 text-sm">
                            <thead>
                                <tr className="border-b border-slate-100 dark:border-slate-800">
                                    <th className="p-4 text-left text-slate-400 font-medium w-1/3 pl-8">{t('coursesOverview.featurePrivilege')}</th>
                                    <th className="p-4 text-center text-slate-500 font-medium w-1/3">{t('free')}</th>
                                    <th className="p-4 text-center text-indigo-600 font-bold w-1/3 bg-indigo-50/30 dark:bg-indigo-900/10">Premium</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {[
                                    { label: t('coursesOverview.textbookAccess'), free: t('coursesOverview.limited3Lessons'), premium: t('coursesOverview.fullAccess1to6') },
                                    { label: t('coursesOverview.topikAccess'), free: t('coursesOverview.limitedRecent'), premium: t('coursesOverview.allPastExams') },
                                    { label: t('coursesOverview.grammarAccess'), free: t('coursesOverview.limited'), premium: t('coursesOverview.unlimited') },
                                    { label: t('coursesOverview.vocabAccess'), free: t('coursesOverview.vocab50'), premium: t('coursesOverview.vocabUnlimited') },
                                    { label: t('coursesOverview.adFree'), free: "—", premium: <span className="text-green-500">✔</span> },
                                ].map((row, i) => (
                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        <td className="p-4 pl-8 font-bold text-slate-700 dark:text-slate-200">{row.label}</td>
                                        <td className="p-4 text-center text-slate-500">{row.free}</td>
                                        <td className="p-4 text-center font-bold text-slate-900 dark:text-white bg-indigo-50/30 dark:bg-indigo-900/10">{row.premium}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
