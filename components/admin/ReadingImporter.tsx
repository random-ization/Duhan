...
import { CheckCircle2, Loader2, Upload, FileSpreadsheet, BookOpen, Music, X, Check } from "lucide-react";
...
const ReadingImporter: React.FC = () => {
    ...
    const bulkImportMutation = useMutation(api.units.bulkImport);
    ...
    // Audio upload state
    const [audioFiles, setAudioFiles] = useState<{ file: File; unitIndex: number; articleIndex: number; status: 'pending' | 'uploading' | 'done' | 'error'; url?: string }[]>([]);
    ...
    useEffect(() => {
        ...
        const colMap = {
            ...
            translationVi: findColumn(['翻译(越)', '翻译(vn)', '翻译(vi)', 'translation(vn)', '越南语翻译']),
                ...
                <div className="bg-white border-2 border-zinc-900 rounded-2xl shadow-[6px_6px_0px_0px_#18181B] p-5 space-y-4">
                    ...
                    <div className="bg-zinc-50 rounded-lg p-3 text-xs space-y-2 border border-zinc-100">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="bg-zinc-200 text-zinc-600 px-1.5 py-0.5 rounded text-[10px]">9 列</span>
                        </div>
