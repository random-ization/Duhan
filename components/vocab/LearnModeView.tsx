...
const LearnModeView: React.FC<LearnModeViewProps> = React.memo(({
  ...
  const generateDistractors = useCallback((correctAnswer: string, count: number = 3): string[] => {
    const pool = allWords.filter(w => 
      (currentQuestionType.includes('K_TO_N') ? w.english : w.korean) !== correctAnswer
    );
    const shuffled = shuffleArray(pool);
    return shuffled.slice(0, count).map(w => 
      currentQuestionType.includes('K_TO_N') ? w.english : w.korean
    );
        ...
        <div className="bg-[#E4DBCF] rounded-2xl p-8 min-h-[500px] flex flex-col relative shadow-sm">
          ...
          <div className="flex-1 flex flex-col items-center justify-center mb-8">
            <div className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
              {currentQuestionType.startsWith('WRITING') ? <Pencil className="w-4 h-4" /> : <AlignLeft className="w-4 h-4" />}
              {currentQuestionType.startsWith('WRITING') ? labels.writingMode : labels.multipleChoice}
            </div>
                ...
                {showFeedback && !isCorrect && (
                  <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <p className="text-sm text-emerald-700">
                      <span className="font-bold">{labels.correctAnswer || 'Correct answer'}:</span> {correctAnswer}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="mt-8 flex justify-center">
            {!showFeedback ? (
              <button
                onClick={checkAnswer}
                disabled={currentQuestionType.startsWith('CHOICE') ? !selectedAnswer : !userInput.trim()}
