            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
              {sessionStats.incorrect.length === 0 ? (
                ...
                sessionStats.incorrect.map((w, i) => (
                  <div key={i} className="flex justify-between bg-white p-3 rounded-lg border border-red-100 text-sm">
          ...
          <div className="bg-emerald-50 rounded-xl p-6">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
              {sessionStats.correct.map((w, i) => (
                <div key={i} className="flex justify-between bg-white p-3 rounded-lg border border-emerald-100 text-sm">
                  <span className="font-bold">{w.korean}</span>
                  <span className="text-slate-600 truncate max-w-[50%]">{w.english}</span>
                </div>
              ))}
