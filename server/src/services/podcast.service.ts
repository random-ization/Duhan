...
export const updateProgress = async (userId: string, episodeGuid: string, progress: number) => {
    try {
        await prisma.listeningHistory.update({
            where: {
                userId_episodeGuid: { userId, episodeGuid }
            },
            data: { progress }
        });
        return true;
    } catch (e) {
        // Record might not exist if trackView wasn't called (shouldn't happen in normal flow)
        return false;
    }
};
